...
- **Battle Test Runner** — `scripts/ouroboros_battle_test.py` boots the full stack: Strategic Direction + Consciousness + Venom + Event Spine + Adaptive Routing + parallel execution (BackgroundAgentPool). Real API cost tracking ($0.50 default cap). Every commit signed `Generated-By: Ouroboros + Venom + Consciousness`.
