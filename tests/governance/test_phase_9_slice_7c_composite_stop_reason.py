...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Composite (the load-bearing case from May 8 soak).
        ("wall_clock_cap+atexit_fallback", True),
...
def test_canonical_shutdown_noise_set_complete():
    """Slice 7c canonical set MUST contain all 5 known
    shutdown-noise stop_reasons."""
    from backend.core.ouroboros.governance.graduation.live_fire_soak import (  # noqa: E501
...
def test_classify_composite_wall_clock_cap_routes_infra():
    """The exact bytes signature from session
    `bt-2026-05-08-022312` MUST classify as INFRA, not RUNNER."""
    from backend.core.ouroboros.governance.graduation.live_fire_soak import (  # noqa: E501
        classify_outcome,
    )
...
def test_classify_uncomposed_shutdown_unchanged():
    """Pre-Slice-7c known-good path: uncomposed `sigterm` /
    `wall_clock_cap` still routes to INFRA — Slice 7c is
    additive."""
    from backend.core.ouroboros.governance.graduation.live_fire_soak import (  # noqa: E501
        classify_outcome,
    )
