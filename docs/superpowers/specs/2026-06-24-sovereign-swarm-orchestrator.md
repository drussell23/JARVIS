...
- **THE GOLDEN RULE (operator-mandated): the worker shape is SYNTHESIZED, never looked up.** The `SubagentFactory` must derive the worker's persona (role label), context budget, and tool allowlist *entirely dynamically* via **AST + semantic inspection of the sub-goal** — NO static enum, NO dictionary of "Agent Roles". The requirements of the problem literally manifest the shape of the worker: parse the sub-goal's `target_files` (AST — what symbols/imports/test-files it touches) + semantically inspect the `goal` text → derive (a) the minimal tool allowlist the work actually needs (read-only tools by default; a mutation tool ONLY if the AST shows the sub-goal mutates a file, and only within budget), (b) a context budget proportional to the inspected scope, (c) a descriptive role label from the work's nature. Reuse the Oracle for the AST/semantic inspection. If two sub-goals need the same shape, that's emergent — never a hardcoded class.
- **`SwarmOrchestrator.define_worker(sub_goal) -> WorkerSpec`** — applies the Golden Rule: AST/semantic-inspect the sub-goal → synthesize the WorkerSpec (role, rendered prompt via `render_worker_system_prompt(role, goal, scope, allowed_tools, mutation_budget, read_only)` — generalize the existing `render_general_system_prompt`, tool allowlist, budgets).
- **`SubagentFactory.build(worker_spec) -> Executor`** — construct the worker executor from the spec: a `ScopedToolBackend(allowed_tools=spec.allowed_tools, max_mutations=spec.mutation_budget)` (already parameterized) + the rendered system prompt + the context budget. **Mutating workers route through the GENERAL executor path (`dispatch_general` + the Semantic Firewall) — zero net-new OS execution surface, the Iron Gate inherited natively** (Decision 2); read-only workers use the EXPLORE path. The factory routes by *synthesized capability* (read-only vs mutating, from the AST inspection), not by a fixed type name.
...
- **Fail-CLOSED:** a worker's allowed_tools is a strict allowlist (ScopedToolGate rejects anything else, pre-policy); mutation_budget is a hard count gate; an over-broad/unparseable spec OR an inspection that can't confidently prove a mutation is needed → fall back to the minimal read-only worker (never a more-capable one). A dynamically-defined worker can NEVER exceed the cage.

### 3.1 Elastic Adaptive Fan-Out (Decision 4)
Base concurrency floor = 3 (the governed default). Beyond that the `SubagentFactory`/scheduler is **elastically adaptive, gated on the EXISTING `SensorGovernor` + `MemoryPressureGate`** — before spawning worker #4 (and each subsequent), it consults live host pressure:
- **Memory/VRAM pressure < 65%** → permitted to **elastically burst** up to `max_concurrency` (env `JARVIS_SWARM_MAX_CONCURRENCY`).
- **Pressure 65–80%** → hold at the current level (no burst, no shrink).
- **Pressure > 80%** → **hard backpressure**: freeze instantiation, hold pending workers in a FIFO queue (no drop, no loss), drain as pressure recedes.
This reuses the `MemoryPressureGate` cascade (psutil → meminfo → vm_stat) + the `SensorGovernor` op-cap — the swarm never exceeds the global governance ceiling; the elasticity operates *within* it. Thresholds env-tunable (`JARVIS_SWARM_BURST_PRESSURE`=0.65, `JARVIS_SWARM_BACKPRESSURE`=0.80). Fail-CLOSED: an unreadable pressure probe → treat as >80% (freeze), never as <65% (burst).
...
- **Total lifecycle isolation (Decision 1):** the `AgentMessageBus` is OWNED by the `ExecutionGraph` — constructed at graph-start, **the bus and its heap CEASE TO EXIST when the DAG completes** (torn down + GC'd in the scheduler's graph `finally`, alongside the §5 sandbox GC). No global/persistent bus, no cross-graph message leakage. Mirror the CommandBus bounded-heap; do NOT duplicate its L1-command semantics. Gated `JARVIS_SWARM_MESSAGE_BUS_ENABLED` (default false).

### 4.1 The Epistemic Deadlock Breaker (Decision 3 — the load-bearing anti-loop guard)
LLM workers fall into polite infinite clarification loops ("thanks, but could you clarify…" forever). The bus enforces a hard turn budget per peer-to-peer clarification:
- Each `CLARIFICATION_REQUEST`↔`CLARIFICATION_RESPONSE` exchange between a given pair (correlation_id) has a strict **`max_turn_budget`** (env `JARVIS_SWARM_CLARIFICATION_MAX_TURNS`, default **3**). The bus counts turns per correlation_id.
- **If the pair reaches turn `max_turn_budget + 1` without a verified artifact produced**, the bus instantly raises a **`DeadlockInterruptedException`**: it (1) **kills both worker processes** (the existing hard-kill wrapper), (2) **dissolves that sub-graph** (cancels the pair's units + their dependents), (3) **bubbles a `[SOVEREIGN YIELD: EPISTEMIC DEADLOCK]`** up to the Fleet Commander **with the full transcript** of the deadlocked exchange (bounded, for the postmortem + the Command Node). Zero infinite loops — structurally bounded.
- Fail-CLOSED: a turn-count read error or an ambiguous "is there a verified artifact" check → treat as deadlock (interrupt), never as "keep talking". The transcript is sanitized (no authority) before it bubbles. The deadlock is a terminal yield (op-never-lost: sealed + bubbled, not silently dropped).
...
## 9. Locked Decisions (operator sign-off 2026-06-24)
1. **Message bus transport — DECIDED: dedicated per-graph `AgentMessageBus` with total lifecycle isolation** (bus + heap cease to exist on DAG completion). See §4 + §4.1.
2. **Mutating-worker engine — DECIDED: reuse the GENERAL executor + Semantic Firewall** — zero net-new OS execution surface; Iron Gate inherited natively. See §3.
3. **Clarification — DECIDED: worker↔worker `CLARIFICATION_REQUEST` allowed, bounded by the Epistemic Deadlock Breaker** (`max_turn_budget`=3 → `DeadlockInterruptedException`: kill both, dissolve sub-graph, bubble `[SOVEREIGN YIELD: EPISTEMIC DEADLOCK]` + transcript). See §4.1.
4. **Fan-out — DECIDED: elastically adaptive, base floor 3, gated on SensorGovernor + MemoryPressureGate** (<65% burst to max; >80% freeze + FIFO). See §3.1.
