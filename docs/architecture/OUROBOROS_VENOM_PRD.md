...
### Forward-Looking Priority Roadmap (rewritten 2026-04-26 post-Pass-C-structural-completion)

This section is the **canonical "what's next" ordering** after Pass B + Pass C structural completion. Reflects the brutal review in §3.6.

**Color legend**: 🔴 = highest priority (do first) · 🟡 = medium priority · 🟢 = least priority (defer).

#### Priority summary table (sortable)

| Rank | Item | Status | Why this rank | Estimate |
|---|---|---|---|---|
| 🔴 **#1** | **Pass C activation pipeline caller wiring + per-loader cadence** (5 loaders to wire) | 🔴 Not started (5/5 loaders shipped 7.1-7.5) | Loaders exist but actual gate-state mutation on `/adapt approve` requires (a) Slice 6 writing approved proposals to `.jarvis/adapted_<surface>.yaml` and (b) callers (orchestrator / `SemanticGuardian._PATTERNS` / `ExplorationFloors` / `ScopedToolBackend` / `risk_tier_floor`) switching to consume the new helpers. | ~3 days wiring + Background cadence |
| 🔴 **#2** | **HypothesisProbe production wiring** (Phase 7.6 substrate complete) | 🔴 Not started (substrate landed) | `AnthropicVenomEvidenceProber` (read-only Venom subset honoring the allowlist) + bridge to `HypothesisLedger.record_outcome` + bridge to `AdaptationLedger.propose` for confirmed hypotheses. | ~3 days |
| 🟢 **(closed)** | **Phase 7.8 — Cross-process AdaptationLedger advisory locking** | ✅ Shipped 2026-04-26 | Closed fragility vector #3 — `fcntl.flock` exclusive lock on `_append`; best-effort no-op fallback on Windows/NFS | — |
| 🟢 **(closed)** | **Phase 7.9 — Stale-pattern sunset signal** | ✅ Shipped 2026-04-26 | Closed fragility vector #4 — `StalePatternDetector` emits advisory `sunset_candidate` proposals; chain-of-responsibility validator composes with Slice 2 | — |
| 🟡 **#5** | **Phase 8 — Temporal Observability** (5 sub-deliverables) | 🔴 Not started | Decision causal-trace + latent-confidence ring + multi-op timeline + flag-changed SSE + latency-SLO breach. Operator-UX-meaningful. | ~2 weeks (parallel to Phase 7) |
| 🟡 **#6** | **Pass B per-slice graduation cadence** (9 flag flips) | 🟡 In flight | Agent-conducted twice-daily soak (`trig_012EvEDkABy2u5PSSs3xK5C4`); ~14 days minimum to flip all 9. | Background |
| 🟡 **#7** | **Pass C per-slice graduation cadence** (~10 flag flips across 6 slices) | 🔴 Not started | 5 clean sessions per slice (Pass C §4.6). Higher bar than Pass B. Starts after Phase 7 caller wiring lands. | Background |
| 🟢 **#8** | **Phase 6 P6 — Self-narrative** | 🔴 Not started | Now blocked by Phase 7 (needs *real* adaptation history, not just substrate). Long-horizon (3-6 months). | ~3 weeks (after Phase 7) |
| 🟢 **#9** | **Skills/workflows surface** (CC parity item) | 🔴 Not started | Nice-to-have UX. No PRD priority assigned. | Future |
| 🟢 **#10** | **Per-action permission granularity** (CC parity item) | 🔴 Not started | Tradeoff against autonomy; likely operator-rejected as anti-vision. | Possibly never |

**Critical-path summary**: Phase 7 (#1-#5) **converts O+V from B− to A−**. Phase 8 (#6) makes the system debuggable at A-level. Phase 6 P6 lands AFTER Phase 7 because self-narrative needs real adaptive history. Note: 5/5 Pass C activation surfaces now have substrate loaders (Phase 7.1-7.5 ✅); the remaining critical-path is **caller wiring** (now Rank #3) + the autonomous-curiosity loop (7.6) + sandbox hardening (7.7).
...
**Caller wiring progress** (post-Phase-7 functional milestone): 2/5 surfaces wired end-to-end:
  ...
  - [x] **Phase 7.2 IronGate floors wiring** — 6 live call sites in `orchestrator.py` (3) + `phase_runners/generate_runner.py` (3) switched from `ExplorationFloors.from_env()` → `ExplorationFloors.from_env_with_adapted()` (this PR). Master-off byte-identical (default-flag state); master-on injects adapted required_categories. 14 wiring pins covering master-off-byte-identical (3) + master-on-injection (4) + caller-grep invariants (3) + caller-authority-no-direct-loader-import (2) + end-to-end smoke (2). Combined regression: **421/421 tests green** across Phase 7.1-7.9 + wiring suite.
  - [ ] **Phase 7.3 ScopedToolBackend per-Order budget wiring** — pending
  - [ ] **Phase 7.4 risk-tier ladder wiring** — pending
  - [ ] **Phase 7.5 ExplorationLedger category-weight wiring** — pending
  - [ ] **Slice-6 MetaGovernor YAML writer** — `/adapt approve` writes `.jarvis/adapted_<surface>.yaml` (pending)
  - [ ] **HypothesisProbe production EvidenceProber** — `AnthropicVenomEvidenceProber` (read-only Venom subset honoring allowlist) + ledger bridges (pending)
...
#### 3.6.2 Five known structural fragility vectors (ranked by severity)

| # | Vector | Severity | Why it matters | Mitigation status |
|---|---|---|---|---|
| **1** | **Sandbox escape via object-graph traversal** in operator-approved candidates. `_SAFE_BUILTIN_NAMES` includes `object` + `type`; once approved, `object.__subclasses__()` walks to `subprocess.Popen` etc. | 🟢 Mitigated (Phase 7.7 shipped) | Was 🔴 Critical pre-2026-04-26. Operator-authorization is no longer the sole defense — `ast_phase_runner_validator.py` Rule 7 hard-rejects candidates that contain `__subclasses__`/`__bases__`/`__class__` Attribute access OR `getattr(x, "<banned>")` string-literal access in any function body, BEFORE the candidate ever reaches the sandbox | 🟢 Mitigated. Per-rule kill switch `JARVIS_AST_VALIDATOR_BLOCK_INTROSPECTION_ESCAPE` defaults TRUE. Known limitation: dynamic-string getattr (runtime-computed attr names) slips through — tracked separately as a much-higher-bar escape requiring the operator to construct the attr name programmatically |
| **2** | **Pass C activation gap** — proposal pipeline shipped; **5/5 boot loaders now landed** (SemanticGuardian / IronGate / per-Order budget / risk-tier ladder / category weights). Caller wiring + Slice-6 YAML writer + per-loader cadence remain | 🟡 Substrate complete; functional gap shifts to callers | All 5 activation surfaces have substrate-complete loaders + helpers + defense-in-depth cage rules. The remaining functional gap is (a) Slice 6 writing approved proposals to YAML on `/adapt approve` and (b) live gates switching to consume the new helpers | 🟡 In-flight via Phase 7 (5/9 slices done — 7.1+7.2+7.3+7.4+7.5 landed; caller wiring is now critical-path) |
| **3** | **Cross-process AdaptationLedger race** — `threading.RLock` + `os.fsync` only; no advisory file lock | 🟢 Mitigated (Phase 7.8 shipped) | Was 🟡 Medium pre-2026-04-26. `fcntl.flock` advisory exclusive lock now wraps `_append` write paths — concurrent processes serialize at the FS level. Best-effort fallback (no-op + log) on Windows / NFS / kill-switch-off | 🟢 Mitigated. Per-feature kill switch `JARVIS_ADAPTATION_LEDGER_FLOCK_ENABLED` defaults TRUE |
| **4** | **Semantic drift over long horizons** — mined SemanticGuardian patterns are additive forever; no sunset signal | 🟢 Mitigated (Phase 7.9 shipped) | Was 🟡 Medium pre-2026-04-26. `StalePatternDetector` now emits advisory `sunset_candidate` proposals when a pattern hasn't matched in N days (default 30). Operator reviews via `/adapt` REPL; actual removal still goes through Pass B `/order2 amend` (advisory-only — Pass C cannot remove patterns) | 🟢 Mitigated. Master flag default false until graduation cadence; chain-of-responsibility validator composes with Slice 2 add_pattern validator without shadowing |
| **5** | **Multi-file APPLY proven once** — Sessions Q-S, 2026-04-15 | 🔴 Critical | 12+ days since last verified landmark. Pass B graduation soak just kicked off | 🟡 Soak in flight via agent-conducted twice-daily cadence |

#### 3.6.3 Critical path to A-level RSI (top 3 systemic upgrades, ranked)

Color legend: 🔴 = highest priority (do first) · 🟡 = medium priority · 🟢 = least priority (defer).

| Priority | Upgrade | Status | Impact-per-week | Why this rank |
|---|---|---|---|---|
| 🔴 **#1** | **Pass C activation pipeline — caller wiring + Slice-6 YAML writer** (5/5 loaders done) | 🟡 5/5 substrate loaders landed (7.1-7.5); caller wiring + Slice-6 YAML writer pending | **Highest**: closes the last functional gap. Transforms Pass C from "loaders exist" → "live gates change behavior on `/adapt approve`" | All 5 activation surfaces now have substrate-complete boot loaders + defense-in-depth cage helpers. The remaining functional gap is wiring the live gates to call them and Slice 6 writing approved proposals to YAML |
| 🔴 **#2** | **Bounded hypothesis-probe loop** (Phase 7.6) — extend HypothesisLedger with read-only Venom probe + structural termination | 🟡 Substrate landed (3 termination guarantees + Null-prober safe-default + 9-value verdict enum); production prober wiring + ledger bridges pending | **High**: closes the autonomous-curiosity gap §3.6.1 row "interleaved reasoning" surfaced | Substrate runner exists with all 3 termination guarantees AST-pinned. Production wiring (`AnthropicVenomEvidenceProber` + ledger bridges) is a follow-up |
| 🟢 **#3** | **Sandbox hardening** (Phase 7.7) — closes vector #1 above | ✅ Shipped 2026-04-26 | **High**: removed the only known structural escape vector by hard-rejecting introspection-attr access at AST validation time | Phase 7.7 added Rule 7 to `ast_phase_runner_validator.py` — blocks `__subclasses__`/`__bases__`/`__class__` Attribute access + `getattr(x, "<banned>")` string-literal access. Per-rule kill switch defaults TRUE |
| 🟡 #4 | **Phase 8 — Temporal Observability** — decision causal-trace ledger + latent-confidence ring + synchronized timeline + flag-changed SSE | Designed (PRD §3.6.4 below) | Medium: meaningful operator UX improvement; not load-bearing for RSI | Enables time-travel debugging on autonomic decisions. ~2 weeks |
| 🟢 #5 | **Cross-process AdaptationLedger advisory locking** — fix vector #3 | ✅ Shipped 2026-04-26 (Phase 7.8) | Medium: safety-net for parallel agent-conducted soaks | `fcntl.flock` advisory locking landed. Per-feature kill switch defaults TRUE |
| 🟢 #6 | **Stale-pattern sunset signal** — fix vector #4 | ✅ Shipped 2026-04-26 (Phase 7.9) | Medium: prevents long-horizon adaptive-surface bloat | StalePatternDetector emits `sunset_candidate` proposals after 30d-no-match. Advisory-only — Pass B amendment still required for removal |
| 🟢 #7 | **Skills/workflows surface (CC parity)** — operator-authored playbooks | Not started | Low: nice-to-have UX; CC parity item | No PRD priority assigned |
| 🟢 #8 | **Per-action permission granularity** — per-tool-use prompt vs per-op risk-tier | Not started | Low: tradeoff against autonomy | Likely operator-rejected as anti-vision |
| 🟢 #9 | **Phase 6 P6 — Self-narrative** | Not started, long-horizon | Low **for now**: blocked by sequencing rule until Pass C is functionally complete (not just structurally) | Becomes high after Phase 7 lands |
...
#### 3.6.5 Honest assessment vs "95%+ sovereign autonomous developer"

**Letter grade: B−.** The vision is A. The execution is B− because:

| Reason | Severity | Closeable by |
|---|---|---|
| Pass C activation gap (substrate vs functional) | 🟡 Substrate complete (5/5 loaders shipped); caller wiring is the remaining functional gap | Phase 7 (#1 above) — caller wiring + Slice-6 YAML writer |
| One verified multi-file APPLY landmark | 🔴 Critical | Sustained soak + new landmark beyond Sessions Q-S |
| Sandbox object-graph escape vector | 🟢 Mitigated (Phase 7.7 shipped 2026-04-26 — AST Rule 7 blocks introspection access) | Phase 7 (#3 above) — was Rank #3 critical-path; now closed |
| Cross-process race (vector #3) | 🟡 Medium | Phase 7 (#5 above) |
| Long-horizon stale-pattern accumulation | 🟡 Medium | Phase 7 (#6 above) |
| No bounded hypothesis-test loop | 🟡 Substrate complete (Phase 7.6 landed); production prober wiring pending | Phase 7 (#2 above) |

**To reach A:**
1. Phase 7 ships (activation pipeline + hypothesis loop + sandbox hardening) — converts Pass C from theater → real
2. Phase 8 ships (Temporal Observability) — operator can debug autonomic decisions
3. ≥ 5 new battle-test landmarks proving sustained multi-file APPLY across diverse domains
4. 90-day soak with operator-approval rate ≥ 70% on adaptation proposals (Pass C arc-closure criterion per `memory/project_reverse_russian_doll_pass_c.md` §10.3)
