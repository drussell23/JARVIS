...
| **§3.6.2 vector #6 Default-False Flag Problem** | §3.6.2 | 🔴 **CRITICAL — Phase 9 cadence operator-runnable as of 2026-05-05** | Critical | Phase 9 substrate IS complete; **runbook now turn-key**: `bash scripts/run_live_fire_graduation_soak.sh` (wrapper exporting all 4 vars including `JARVIS_GRADUATION_LEDGER_ENABLED` parent-process fix from 2026-05-05) OR `bash scripts/install_live_fire_soak_cron.sh --once` for first proof, OR `--install` for ongoing cron cadence. Single-source-of-truth pin in `test_install_live_fire_soak_cron.py` enforces all 4 env vars stay synchronized across cron generator + --once + wrapper + crontab example (5 new structural pins; 21/21 tests). Operator runs the soaks → `.jarvis/live_fire_graduation_history.jsonl` + `.jarvis/graduation_ledger.jsonl` accumulate evidence → 12+ flag × 3-clean-session ladder closes the brutal-review's critical blocker |
...
| 11 | Session search via SQLite index | new `session_archive.py` | ~4–5h | pending |
| 12 | Op dependency graph / parallel fan-out canvas | `op_block_buffer.py` ext | ~5h | pending |
...
| 14 | Operation modes (`/plan` `/analyze` `/apply` `/auto`) | §32.7 Pattern B | ~1 slice | pending |
...
| 16 | Per-component tool scope (Pattern C) | §32.7 C | ~2 slices | pending |
