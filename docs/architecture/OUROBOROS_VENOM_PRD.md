...
>
> 3.34 (2026-07-22 — **The Four-Soak APPLY Campaign: execution grade C+ → B−/B; first verified autonomous fix on the current stack.** Four instrumented soaks (bt-2026-07-21-205755 → bt-2026-07-22-022146) drove the failure frontier from CLASSIFY to a landed, 2PC-verified mutation. **What failed and was killed, in order**: (1) cold-cache advisor blast-radius FABRICATION (`blast=50` presented as measured; cache-poisoned; a Slice 21 Fix B bypass on the cooperative path) → Targeted Locality Bounding (`advisor_locality.py`, O(K) neighborhood via reverse_dep_resolver) + Epistemic Humility (provenance `unknown` = neutral + NOTIFY_APPLY floor on BOTH GATE paths) — two soaks since: 0 blocks, 0 fabrications (#70003); (2) APPLY ENOENT on a write-root-DOUBLED path → Path Canonicalization Sandbox (`canonicalize_candidate_path`, iterative longest-root-suffix alignment + `PathTraversalError`) (#70004); (3) the universal target-existence gate wired ONLY on the inline twin while `generate_runner.py` (shipping since 2026-04-23) had ZERO target-guard code — the wired-but-inert trap — plus WAL-replayed ghost signals chasing shield-stashed untracked files → shipping-path Gate 3.5 + WAL Signal Liveness Tombstoning + Sandboxed Ephemeral Instantiation (#70005); (4) the per-file freeze budget rolling back its own multi-file success → Transactional Op-Scoped Freeze (latch tracks base op_id; siblings complete 2PC; Bounded Fan-Out ceiling 5; oversized batches rejected pre-write). **What worked**: DW 397B RT lane end-to-end (`service_tier=priority`, TTFT p50 6.5–8.9s vs the 66s async class, $0.016–0.019/soak, zero Claude spend); Atomic Flush & Freeze captured + flushed + parked the first mutation on its first live encounter; the organism CORRECTLY fixed the planted `soak_probe_math` defect (diff sha8 84a20ef2). **Runner-parity audit**: Slice 78 PatchDomainGuard was ALSO inline-only → ported to the shipping runner. **Path to A−/A+**: net-landed multi-file mutations under the op-scoped latch → AutoCommit + workspace promotion exercised (`JARVIS_WORKSPACE_PROMOTION_ENABLED` arm-at-launch) → multi-op sessions without idle-out (~20-min op-flow dry-up is the throughput ceiling) → sustained multi-session cadence with human-reviewed parked diffs. Composite assessment: execution tier no longer the bottleneck-of-unknowns — every remaining gap is enumerated.
>
...
>
> 3.30 (2026-06-20 — **§3.10 added — Sovereign Aegis Batch-Passthrough Matrix** (first autonomous `state=applied` on cloud through the live credential proxy on BOTH transports; corrects the misattributed "Aegis ruptures DW" belief — two isolation soaks prove RT/SSE AND batch lanes healthy with shields up. The only blocker on MASSIVE payloads was SIZE via three stacked ceilings — aiohttp `client_max_size`=1MB (outermost, non-obvious), passthrough cap 4MB, provider preflight 5MB. Fix = constant-memory `stream_body_capped` passthrough + aligned 64MB caps + raised aiohttp ceiling; `AEGIS=true`+`DYNAMIC=true` armed, force-batch tourniquet removed. Live: autonomous op self-routed dynamic→batch→aegis→`state=applied`, 4xx/5xx/toolarge=0. 274 tests; PR #69604). — see §3.10 below)
>
...
> - **[`OUROBOROS_VENOM_PRD_SPEC_DETAIL.md`](./OUROBOROS_VENOM_PRD_SPEC_DETAIL.md)** — §24–§50 (brutal architectural reviews, per-phase requirements).
> - **[`OUROBOROS_VENOM_NORTH_STAR.md`](./OUROBOROS_VENOM_NORTH_STAR.md)** — §51 North Star Galaxy + the §51.11.x slice-arc changelog narratives + the **active next-slices roadmap (§51.11.34-ROADMAP)**.
> - **[`OUROBOROS_VENOM_PRD_HISTORY.md`](./OUROBOROS_VENOM_PRD_HISTORY.md)** — full version-history headers + §40/§41 prior-context closure banners + cross-off tables.
...
- [~] **M12 — `JPrimeLoRA`** (long-horizon, **operator-gated**): closes in-weight lifelong-learning axis (only viable on J-Prime — Claude/DW are inference-only). Requires data curation pipeline + training harness + eval harness + route gating. **Data curation + eval harness SHIPPED 2026-09-04 (PRD v3.35)**: the trajectory recorder now persists every ANSWER (patch / refusal / unparseable) as a gradable row, reactor's `grpo_preflight` + `grpo_verifier` are the eval harness (Gate 3, `--min-groups`), and `run_grpo_training.py` + `memory_guard` are the trainer runner. Target is LOCAL `Qwen3-Coder-30B-A3B-Instruct` (bnb-NF4 at load) rather than GCP — the corpus was generated BY that model, so it is the one the corpus can teach. **Still open**: no training run has executed; corpus at 20 trainable groups vs a 25 target; route gating unbuilt. **Sequencing caveat**: this is Phase 11 work done AHEAD of Priority #1 (Live-Fire Graduation Cadence), at operator direction. §30.6 is a stale cross-reference — that section now covers multi-cockpit, not the M12 priority decision.
...
**Phase 12 — Systemic Upgrades v3 (Bounded Epistemic Loop + DecisionRecord Causality Graph + Failure-Mode Memory)** *(NEW 2026-05-04 — derived from §31)*: status — ✅ **ALL 3 UPGRADES CLOSED via §31 same-day burst 2026-05-04** (see v2.12 + v2.15 banners). Closes per-op information adequacy + cross-session decision reproducibility + cross-op pattern accumulation loops respectively. **Stale "NOT STARTED" framing refreshed 2026-05-07 (v2.50 banner)** — section header was authored alongside §31 scoping but the items shipped same-day.
...
| ✅ **#2** | **Adversarial Cage Stress — empirical Quine-resistance numbers** | ✅ Shipped 2026-04-27 (Phase 9.4) — row synced 2026-05-09 (v2.82) | Phase 9.4 shipped `graduation/adversarial_cage.py` (~530 LOC) + 38-entry corpus (`tests/governance/adversarial_corpus/corpus.py`) — pure-evaluation harness running candidates through the FULL static cage (`validate_ast` + `SemanticGuardian` + `ScopedToolBackend`). **0/38 open vectors** (real regressions), **12/38 documented known gaps** tracked in PRD §3.6.2 vector #7, 6/6 clean controls passed, 0 harness errors. Pass-through rate 12/38 = 31.58% (every passing entry is documented). Closure pattern mirrors Vector #10 sync — substrate was real but table row was stale. 36/36 regression tests green. | ~30 min row sync (substrate already shipped) |
| ✅ **#3** | **Cross-Session Coherence Harness + Phase 8 Producer Wiring** | ✅ FULLY CLOSED 2026-05-09 — Part A (v2.80) + Part B (v2.81) | **Part A — Cross-session coherence harness ✅**: new substrate `governance/cross_session_harness.py` (~970 LOC) composing the 4 canonical cross-session memory surfaces (UserPreferenceStore + AdaptationLedger + SemanticIndex + LastSessionSummary). Closed 4-value `CoherenceAxis` enum (one per substrate) + closed 4-value `DriftLevel` enum (STABLE/DRIFTING/DIVERGED/CORRUPTED). Frozen §33.5 `AxisDigest` + `CrossSessionDigest` + `AxisDrift` + `CoherenceReport` artifacts. `aggregate_digest(project_root)` computes per-axis sha256-pinned fingerprints (deterministic on same inputs). `compute_drift(before, after)` classifies field-level deltas first-match-wins (additive growth → STABLE; deletion → DRIFTING; same count + different hash → DIVERGED; load failure → CORRUPTED). `simulate_session_boundary()` resets canonical in-process singletons (UserPreferenceStore + LastSessionSummary defaults) so the next digest reads from disk rather than stale cache. `report_coherence((d1, d2, ..., dN))` walks N-session arc and produces `overall_stable: bool` (every axis at every boundary lands STABLE). 6 AST pins (master_default_false / axis_taxonomy_4 / level_taxonomy_4 / **composes_all_4_substrates** / **digesters_cover_all_axes** / authority_asymmetry). **40 regression tests** in `tests/governance/test_vector_5_cross_session_coherence.py`: master flag default-FALSE + 5 truthy + 4-value axis + 4-value level + frozen artifacts to_dict + axis lookup + aggregate master-off / master-on returns 4 axes / **deterministic on same root** + 7 compute_drift parametrized scenarios (both empty / identical / additive / deletion / same-count-different-hash / diagnostic-corrupted / axis-mismatch) + **3-session arc with user-pref growth lands STABLE** + **deletion produces DRIFTING level + overall_stable=False** + zero-session trivially stable + simulate_session_boundary resets singletons + 6 AST pin canonical-source pass + 6 synthetic regressions + 2 canonical-source smokes (`EVENT_TYPE_COHERENCE_REPORTED` registered + all 4 substrates importable). New canonical SSE event `EVENT_TYPE_COHERENCE_REPORTED`. **Test results 1124/1124 cumulative** across §38.11 + §39 + Wave 3 hygiene + Vector #5 + canonical sources. **Part B (Phase 8 producer wiring) still open** — separate architectural concern; Part A ships the validation infrastructure that Part B will USE to prove cross-session signal carryover. The empirical 50+ session arc remains future operator-paced work; Part A ships the rails. Maps to fragility vector #5 (Semantic drift over long horizons). | Part A: closed; Part B: ~1 week |
| ✅ **#4** | **Cross-runner artifact contract (schema-versioned)** | ✅ Shipped 2026-05-09 (v2.85) — single substrate `governance/artifact_contract.py` (~530 LOC) + dispatcher choke-point wiring | Closed 10-value `ArtifactKind` taxonomy mirroring PhaseContext slots 1:1 + closed 6-value `ValidationOutcome` (UNKNOWN_KEY / TYPE_MISMATCH / WRONG_PRODUCER / SCHEMA_VERSION_SKEW / PASSED_DISABLED / OK). Frozen `ArtifactSpec` registry of 10 entries with per-artifact `schema_version` + duck-typed `validate_value` callable + producer/consumer phase sets. Pure-function `validate_artifact_value` + `validate_artifacts_bundle` + `first_failure` — NEVER raises. Wired at SINGLE choke point `phase_dispatcher.py:828` BEFORE `pctx.merge_artifacts(...)` so unknown keys are rejected before they silently land in `extras`. Master flag `JARVIS_ARTIFACT_CONTRACT_ENABLED` (default-FALSE) + `JARVIS_ARTIFACT_CONTRACT_STRICTNESS` (advisory/strict). Catches 4 violation classes: rename/typo, type-shape drift, phase-ownership confusion, schema version skew. 32 regression tests + AST pin asserting validator runs BEFORE merge_artifacts (positional pin) + AST pin asserting registry covers every PhaseContext slot. 1147/1147 cumulative green. **Last structural engineering arc** — autonomy roadmap now transitions to operator-paced Phase 9 cadence. |
...
| 🟡 **#6** | **Mask-discipline regression sweep** | ✅ SHIPPED 2026-05-09 (v2.78) | Wave 3 v2.25 closed the SUBSTRATE — `FlagChangeEvent.to_dict()` masks credential-shaped flag names via sha256[:8]+length-token. **§35 #6 closure** (Vector #9 broader): structural AST regression sweep walks `backend/`, detects every `.prev_value` / `.next_value` Attribute access + every `getattr(_, "prev_value"\|"next_value")` Call, asserts the receiver file is on a bytes-pinned 2-entry allowlist (`flag_change_emitter.py` substrate + `sse_bridge.py` consumer using getattr+`_mask_flag_value` double-mask). Drift requires reviewer attention + explicit allowlist update. **5 new regression tests** — `test_mask_discipline_consumer_chain_pinned` + `test_allowlist_files_actually_use_the_fields` (anti-stale) + `test_allowlist_size_pinned` (forces reviewer attention on size change) + `test_substrate_uses_mask_helper` (defense-in-depth pin on the `_mask_value` + `_is_sensitive_flag` substrate) + `test_sse_bridge_pipes_through_mask_helper`. IDE route stale comment updated post-Wave-3 (it now correctly cites the substrate's name-pattern mask + the route's stricter presence-only double-mask). 23/23 mask-discipline tests + 1081/1081 cumulative. Maps to fragility vector #9. | ~1 day |
| 🟡 **#7** | **AutoCommitter cross-process flock** | ✅ SHIPPED 2026-05-05 (Wave 3 v2.26) + STRENGTHENED 2026-05-09 (v2.79) | Wave 3 closure: new `async_flock_critical_section` async-safe primitive in `cross_process_jsonl.py` (composes existing sync `flock_critical_section` via `asyncio.to_thread` enter/exit + persistent `ExitStack`). `AutoCommitter.commit()` extracts TOCTOU body into `_commit_critical_section`, wraps in per-intent_token flock at `<repo>/.jarvis/auto_commit_locks/<token[:32]>.lock`. Different intent_tokens → different lock files (no cross-op blocking); same token → same path (TOCTOU closed). `commit_lock_contended` skipped_reason for audit. Substrate-unavailable fallback preserved (NEVER raises). **2026-05-09 strengthening adds 3 missing-depth tests**: (a) `test_async_flock_serializes_across_processes` — true cross-process race via `multiprocessing.Process` spawning child that holds `flock_critical_section` for 1.2s; parent attempts `async_flock_critical_section(timeout_s=0.3)` MUST fail; then verifies acquisition succeeds after child release (proves OS-level fcntl lock works, not just within-process asyncio.Lock); (b) `test_auto_committer_returns_commit_lock_contended_on_timeout` — monkey-patches `async_flock_critical_section` to yield `False`, asserts `CommitResult(committed=False, skipped_reason="commit_lock_contended", intent_token=...)` (proves contention-handling code path is wired); (c) `test_auto_committer_substrate_unavailable_falls_through` — hides the substrate module via `sys.modules[…]=None`, asserts `_commit_critical_section` legacy path is invoked (proves fallback decision is wired). Maps to fragility vector #10. **8/8 Vector #10 tests + 1084/1084 cumulative.** | ~1 day |
| 🟡 **#8** | **CuriosityScheduler monotonic-clock conversion** | ✅ SHIPPED 2026-05-09 (v2.77) | Rate-cap window uses `time.time()` (wall clock). Same vector that bit HypothesisProbe (since fixed). Clock skew (ntpd update mid-session) skips cap entirely or triggers spurious throttle. Convert to `time.monotonic()`. Maps to fragility vector #11. **Closure**: `CuriosityScheduler.tick()` now accepts `now_mono` kwarg + dual-tracks `_fire_history_mono` + `_last_fire_mono` for gating; wall-clock preserved as `ts_epoch` audit field; `seconds_since_last_fire` reports honest monotonic delta. 10 new regression tests (TestVector11MonotonicGating + TestVector11SourcePins) — `tests/governance/test_curiosity_scheduler.py:Section J`. 56/56 scheduler tests + 1033/1033 cumulative across §38.11 + §39 + scheduler. | ~2 hours |
...
| **10** | **AutoCommitter race on same op_id** | ✅ CLOSED 2026-05-05 (Wave 3 v2.26) + strengthened 2026-05-09 (v2.79) | Three times empirically observed in single dev session: background AutoCommitters raced with foreground commits on same op_id producing overlapping commits | ✅ Closed. Substrate: `async_flock_critical_section` primitive + `AutoCommitter._commit_critical_section` TOCTOU wrap + per-intent_token `.jarvis/auto_commit_locks/<token>.lock`. 2026-05-09 strengthening adds true cross-process race coverage + contention-path test + substrate-unavailable fallback test. See §35 row 🟡 #7 |
...
|---|---|---|---|---|
| 🟡 **#1** | **Live-Fire Graduation Cadence — convert 12+ flags `false` → `true`** | 🟡 Substrate FULLY FIXED (v2.86 + v2.87 + v2.88 close all 6 layers); evidence ladder pending operator-paced soaks | **HIGHEST**: the architecture is built; now prove it works under real model output. Until ≥3 clean sessions per substrate flag complete, the cognitive loop is theoretical. Subsumes Phase 8 producer wiring on the road. **Cadence arc closures 2026-05-10**: v2.86 Layers 1-4 (.env loading via wrapper / --once delegation / JARVIS_TOPOLOGY_SENTINEL_ENABLED default in cadence path / socksio pinned for httpx[socks] under sandbox-tunnel agent envs); v2.87 Layer 5 (modality ledger separated from verification probing — disk ledger now always loads, classifier consults pre-classified verdicts so TERMINAL_OPEN models don't lead candidate ranking); v2.88 Layer 6 (wall-clock watchdog Layer 4 escape-hatch writes partial summary.json synchronously before os._exit(75) — composes existing _atexit_fallback_write from Wave 3 v2.79 + adds incomplete_kill_layer4 outcome marker). The substrate is ready: next soak produces parseable evidence (cost-positive ops or structured runner-failure record), modality ledger informs catalog ranking with canonical Qwen3.5-397B at top, wall-clock cap exits leave linkable session_id even when asyncio loop wedges. | Cron-driven daily soak harness running 3 sessions/day rotating which substrate flags are enabled, producing graduation-ledger evidence per Item #4. ~3 flag flips/week × 24 flags ≈ 6-9 weeks operator-paced wall-clock. **Architecturally complete; waiting on cadence runs.** |
| ✅ **#2** | **Adversarial Cage Stress — empirical Quine-resistance numbers** | ✅ Shipped 2026-04-27 (Phase 9.4) — row synced 2026-05-09 (v2.82) | **CLOSED**: 0/38 open vectors, 12/38 documented known gaps (each tracked in §3.6.2 vector #7 with explicit ticket), 6/6 clean controls passed. See §35 row #2 for substrate details. |
| ✅ **#3** | **Cross-Session Coherence Harness + Phase 8 Producer Wiring** | ✅ FULLY CLOSED 2026-05-09 — Part A (v2.80) + Part B (v2.81) | **HIGH**: foundation for Second Order RSI. Cross-session memory (LSS / SemanticIndex / UserPreferenceMemory / AdaptationLedger) exists; Part A ships the harness substrate. | **Part A ✅**: substrate `cross_session_harness.py` composes all 4 canonical surfaces; closed 4-value `CoherenceAxis` + `DriftLevel`; deterministic per-axis sha256 digests; pure-function `compute_drift(before, after)` classification; `simulate_session_boundary` resets in-process singletons; `report_coherence` produces multi-session arc digest. 6 AST pins + 40 regression tests. See §35 row 🟡 #3 for full details. **Part B (Phase 8 producer wiring) still open** — separate architectural concern: orchestrator ROUTE phase calling `decision_trace_ledger.record()`, classifiers calling `latent_confidence_ring.record()`, phase-timing calling `latency_slo_detector.record()`. Without producers, the 8 Phase 8 GETs + 5 SSE events return empty |
| ✅ #4 | **Cross-runner artifact contract (schema-versioned)** | ✅ Shipped 2026-05-09 (v2.85) | New substrate `governance/artifact_contract.py` (~530 LOC) closes fragility vector #8. Closed 10-value `ArtifactKind` taxonomy mirroring PhaseContext slots 1:1 + closed 6-value `ValidationOutcome`. Frozen `ArtifactSpec` registry (10 entries with per-artifact schema_version + duck-typed validate_value + producer/consumer phase sets). Wired at SINGLE choke point `phase_dispatcher.py:828` BEFORE `pctx.merge_artifacts` (positional invariant AST-pinned). 4 violation classes: UNKNOWN_KEY rename / TYPE_MISMATCH shape drift / WRONG_PRODUCER phase ownership / SCHEMA_VERSION_SKEW. Master flag default-FALSE + strictness env (advisory/strict). 32 regression tests. See §35 row 🟡 #4 for full details |
| 🟡 #5 | **Mask-discipline regression sweep** | ✅ Shipped 2026-05-09 (v2.78) | Substrate already masked (Wave 3 v2.25 — `to_dict()` + `_SENSITIVE_NAME_TOKENS`); broader closure ships AST allowlist-pin against future direct-field-access consumers — see §35 row #6 |
| 🟡 #6 | **AutoCommitter cross-process flock** | ✅ Shipped 2026-05-05 (Wave 3 v2.26) + strengthened 2026-05-09 (v2.79) | Medium: empirically observed race 3× in single dev session | Closed via `async_flock_critical_section` primitive + `_commit_critical_section` TOCTOU wrap — see §35 row 🟡 #7 |
| 🟡 #7 | **CuriosityScheduler monotonic-clock conversion** | ✅ Shipped 2026-05-09 (v2.77) | Medium: latent vector that bit HypothesisProbe (since fixed) | Closed via `time.monotonic()` dual-tracking — see §35 row #8 |
...
| Cross-session coherence never validated | ✅ Closed 2026-05-09 (v2.80 Part A + v2.81 Part B) | Priority #3 (Coherence Harness + Phase 8 Producer Wiring) — closed |
...
**Two empirical landmines I'd patch first**:
  ...
  2. ~~**The AutoCommitter race on same op_id.** Documented in vector #10. Empirically observed three times in single dev session (this conversation context). Apply Phase 7.8 flock pattern.~~ ✅ **Closed 2026-05-05** (Wave 3 v2.26 — `async_flock_critical_section` primitive + `_commit_critical_section` TOCTOU wrap with per-intent_token lock at `.jarvis/auto_commit_locks/`). Strengthened 2026-05-09 (v2.79) with true cross-process race coverage + contention-path test + substrate-unavailable fallback test (8/8 Vector #10 tests).
...
### 3.10 Sovereign Aegis Batch-Passthrough Matrix — autonomous `state=applied` with shields up *(NEW 2026-06-20)*

**The first autonomous `state=applied` on cloud through the live security proxy on BOTH transports — and the correction of a long-misattributed diagnosis.** The `docker-compose.gcp.yml` cloud overlay had carried `JARVIS_AEGIS_ENABLED=false` on the belief that re-arming the Aegis credential proxy *ruptured* the DW streaming path. Two isolation soaks on an 8-vCPU GCP Spot node (`jarvis-ouroboros-soak-20260619-230622`) disproved it on both lanes:

- **RT/SSE-through-Aegis is healthy.** Aegis ON + RT transport: `[AegisForward] credential injection` on `/v1/chat/completions`, `upstream_status=200` (zero 4xx/5xx), zero `Unclosed connection` (the #69590 leak fix holds), `rupture_risk=False` throughout, `op-019ee41b → LEDGER_TERMINAL state=applied`. The boot `/v1/models 401`s are transient "credential proxy warming" that self-heal to 200 in ~3 min — fail-soft, not a generation failure. Since `env_scrub` pops `DOUBLEWORD_API_KEY` from the loop's env when Aegis is on, that op *structurally could not* have reached DW except through the proxy.
- **Batch-through-Aegis is also healthy** — it was never "ruptured." A forced-batch soak ran the FULL proxied lifecycle clean: `POST /v1/files` (multipart upload) → `POST /v1/batches` → `GET /v1/batches/{id}` (poll) → `GET /v1/files/{id}/content` (retrieve), all `upstream_status=200`, → `state=applied`. What actually regressed the earlier *dual* soak was never the proxy stream; it was the now-fixed webhook-blocking-poll bug (#69599) plus the boot-warming 401 race.

**The one real blocker on MASSIVE payloads was SIZE — three stacked ceilings 413'd a big batch JSONL before it reached DW**, the outermost of which was non-obvious:

1. **aiohttp `web.Application` default `client_max_size` = 1 MiB** — enforced by the *server* before handler code runs, hit first. (`request._client_max_size` propagates correctly via `Application(client_max_size=)` → `_make_request`; but `aiohttp.test_utils.TestServer` and any stub upstream keep the 1 MiB default, which had masked this in tests.)
2. Aegis passthrough body cap default = 4 MiB.
3. DW provider upload preflight default = 5 MiB.

The **Sovereign Aegis Batch-Passthrough Matrix** (PR #69604) removes all three structurally and makes the lane non-blocking — no tourniquet:

- **`request_body.py`** — `stream_body_capped()`: a constant-memory async generator that drains `request.content` and yields chunks straight to the outbound request (no double-buffering a multi-MiB JSONL), enforcing the cap mid-stream; `content_length_hint()` enables a clean early 413 from the declared size before a byte is read.
- **`passthrough.py`** — streaming forward path, default-on (`JARVIS_AEGIS_STREAM_PASSTHROUGH` kill switch → byte-identical buffered fallback). Body cap default 4 MiB → **64 MiB**. GET poll/retrieve unchanged (no body).
- **`daemon.py`** — `web.Application(client_max_size = cap + 1 MiB)` so aiohttp's server ceiling no longer 413s massive uploads pre-handler, sitting just above the body cap so the passthrough's clean JSON 413 stays authoritative.
- **`doubleword_provider.py`** — upload preflight 5 MiB → **64 MiB**. INVARIANT: **provider cap ≤ Aegis cap ≤ aiohttp ceiling** (provider rejects first with a precise cause).
- **`docker-compose.gcp.yml`** — `AEGIS_ENABLED=true` + `DYNAMIC_TRANSPORT=true` (the dynamic `PayloadTransportHeuristic` is ARMED, the force-batch pins are GONE), with explicit 64 MiB caps + streaming for auditability.

**Validation.** 274 Aegis + slice37 tests green; the routing decision itself is covered reproducibly by `tests/governance/test_payload_transport_heuristic.py` (`test_large_file_batches` ≥400 lines → batch, `test_multi_file_batches` ≥3 files → batch). Live heavy-duty soak on the GCP node with shields up + router armed: autonomous **op-019ee444** self-routed dynamic → **batch** (`File upload START payload=18730 bytes`), streamed through `AegisPassthrough` `/v1/files` → `/v1/batches` → poll ×7 → `/v1/files/{id}/content` (all upstream 200), `Batch 2a00461e completed` (await 31s, retrieve 543ms), `LEDGER_TERMINAL state=applied`, `4xx=0 5xx=0 toolarge=0`. The live op's JSONL was 18 KB (integration proof: dynamic→batch→aegis→apply); the **>4 MiB streaming capacity** is proven by the unit tests (2 MiB streamed byte-identical through the real daemon + a 128 MiB-stub, over-cap clean 413 pre-upstream, 64 MiB cap). Net: the engine can now **autonomously route a large op through the batch lane, push it through the live credential proxy, and apply it — shields up, no tourniquet** — directly serving the "rewrite massive architectures autonomously" capability. *(Operational note: the GCP node is a Spot instance with `max_run_duration` auto-DELETE; it was reclaimed after the soak captured the result — cost-bounded by design.)*

---

### 3.11 Sovereign Cognitive Graduation Crucible — the autonomic self-upgrading loop *(NEW 2026-06-20)*

**The pivot to PRD Priority #1 (Live-Fire Graduation Cadence) — executed by hardening + igniting the EXISTING substrate, not rebuilding it.** A four-stream audit established that the autonomous graduation machinery is ~95% already shipped and gated off: `flag_registry` (dynamic flag+default discovery), `graduation_ledger.CADENCE_POLICY` (per-flag required-clean-sessions), `phase9_orchestrator.next_recommended_flag` (what-to-soak-next ranking), `graduation/live_fire_soak.py` (forks `ouroboros_battle_test.py`, `DEFAULT_SEED_INTENTS_PER_SOAK=3`, records each soak via `graduation_ledger.record_session`), `graduation_contract.arbitrate` (anomaly>recovery>predicate>blocklist priority matrix), `telemetry_parse.parse_metrics`, `autonomous_graduation_engine.{evaluate,execute}_graduations` (Gate A eligibility + Gate B AST stability + Gate C contract), and the `sleep_daemon` idle driver that *already calls* `execute_graduations()` when the engine flag is on. Building a parallel scanner/crucible would have duplicated five tested subsystems — rejected.

The audit isolated a genuine ~5% gap, closed across 5 slices (PRs #69606 + #69608; all paths gated default-off + fail-soft + byte-identical when off):

- **Slice 1 — TTFT/AST math-veto** (`graduation/crucible_verdict.py` + `telemetry_parse` fields + `graduation_contract.predicate_cognitive_graduation`). The clean-predicate already bucketed FSM-exhaustion faults (`phase_runner_error`/`candidate_validate_error`/`fsm_state_corruption`) but had no latency or structural-integrity gate. New env-tunable veto: TTFT degradation (absolute ceiling `JARVIS_CRUCIBLE_TTFT_CEILING_MS=30000` + ratio-vs-EWMA-baseline `JARVIS_CRUCIBLE_TTFT_DEGRADE_RATIO=1.5`) + explicit AST corruption (`PhaseRunnerASTValidationError` / `ast_*`/`syntax_*` failure_class / placeholder_detected). **Veto-only + fail-open** by construction — it only ever *downgrades* a CLEAN outcome on positive harm, so it never undoes the arbiter's recovery-override (a regression caught + fixed during the build).
- **Slice 2 — Sovereign Telemetry Manifest** (`graduation/telemetry_manifest.py`). The `[SOVEREIGN GRADUATION]` PR body is machine-generated + deterministic: a per-soak evidence table (TTFT target/actual/verdict, AST signals/verdict, FSM recovery), an aggregate verdict (clean-soak count, TTFT-vs-ceiling, AST integrity, FSM-exhaustion rate), a "Why merge" argument, a 🛟 **Rollback Strategy** payload (instant env hot-revert + exact `git revert`), and a sha256[:16] tamper-evident evidence digest. `manifest_recommends_merge()` is the pure gate the crucible consults before proposing.
- **Slice 3 — source-of-truth PR** (`graduation/graduation_pr_proposer.py` + `orange_pr_reviewer` `body_override`/`title_override`). `flip_default_to_true` is a PURE, AST-validated rewriter that flips the `os.environ.get` default literal `false→true` for exactly the named flag — refusing on 0 matches, >1 match (ambiguous → abstain not guess), already-truthy, or `ast.parse` failure. **Cage-respecting: proposes a PR, never self-merges** (the Order-2 cage's `amendment_requires_operator` is locked-true). Mutates the definitive Python source, NOT a brittle `.env`.
- **Slice 4 — wiring** (`live_fire_soak` folds `crucible_evidence` into `EvidenceRow.telemetry`; `recent_clean_crucible_evidence` reads the last 3 CLEAN soaks; `execute_graduations` AUTO_FLIP fires `_maybe_propose_source_pr`, gated `JARVIS_CRUCIBLE_GRADUATION_PR_ENABLED`). Closes the loop on the existing `sleep_daemon` path.
- **Slice 5.5 — literal-site locator** (`locate_default_literal_file`). First-ignition dry-run surfaced that `FlagSpec.source_file` points to a flag's declaration/doc site, not its default-literal site (only 6/96 align). The locator searches the codebase (bounded, skips `.git`/`.venv`/`.claude`/`tests`) for the single flippable falsy default; hint (source_file) wins if valid; abstains on zero/ambiguous. Lifts clean resolution **6 → 10** flags; the other 87 correctly abstain (indirect accessors / multi-site reads — conservative, no wrong rewrites).

**First Cognitive Ignition (deterministic, against real repo code).** With the gate on and realistic 3-clean-soak evidence, the crucible end-to-end: discovered the real `JARVIS_EXPLORATION_LEDGER_ENABLED` literal site (`exploration_engine.py`, despite the registry pointing at `exploration_ledger.py`), rewrote the source-of-truth default `"" → "true"` (AST-valid, single match), rendered the full Telemetry Manifest (3/3 soaks PASS, TTFT 842ms max 1190ms vs 30000ms ceiling, 0 AST signals, 0% FSM-exhaustion, digest `e8ad507b71c5fd88`, why-merge + rollback), and titled it `[SOVEREIGN GRADUATION] Activated JARVIS_EXPLORATION_LEDGER_ENABLED` — opening a PR (would-merge stays the human/OCA gate). 38 crucible tests + 200+ graduation/contract/soak/orange tests green.

**Unattended cloud cadence (the remaining real-clock work).** The deterministic ignition proves the machinery; the *autonomous-soak* path is now armed and is a genuine multi-hour real-clock process: per flag it requires 3 fresh CLEAN soaks (~15-40 min each via the existing `install_live_fire_soak_cron.sh` / `sleep_daemon`), the node needs `gh auth` for the PR, and a Spot node risks reclamation across that window. The crucible flips no flag without that evidence clearing the math-veto.
...
6. every verb declaring authored help answers when DISPATCHED (§27.5.6) —
   an execution pin, because a search pin cannot see a dynamic mount.

---

### 27.5.6 A verb mounts by filename — and grep cannot see it

`battle_test/repl_dispatch_registry.py` auto-discovers every module-level
`dispatch_<verb>_command` across the curated provider packages and routes
`/<verb>` to it. For `X_repl.py` the verb is `X`; for `pkg/repl.py` it is
`pkg`. `serpent_flow` awaits `try_dispatch` before its legacy ladder, and
`repl_completion._describe` reads a module's authored `__verb_help__` as the
highest-priority description source, short-circuiting the arbiter. **88 verbs
route this way today.** A new `*_repl.py` is reachable and documented the
moment it lands, with no registration edit anywhere.

> **Correction (2026-08-15).** An earlier revision of this section described a
> second mechanism — a "naming cage" — introduced on the premise that `/reach`
> and `/why` had shipped *"importable, tested, and untypeable."* **That premise
> was false.** Both were routed by `repl_dispatch_registry` the whole time. The
> diagnosis came from grepping for the verbs' names and finding no caller, and
> **a dynamic mount is invisible to grep by construction** — the same blind
> spot §28's reachability audit exists to correct, applied to the audit's own
> author. Because `try_dispatch` runs ~300 lines earlier in the same ladder,
> the added mechanism never fired: a module built to fix unmounted features
> that was itself unreachable. It has been removed; nothing was folded forward
> because every capability it offered already existed, including the
> `__verb_help__` short-circuit it was supposed to contribute.
>
> Two pieces of contradicting evidence were available and misread: `/reach` was
> already in the completion palette, and the modules' own docstrings said
> "auto-discovered through the naming cage" — which was **true**, naming the
> registry.

**The rule this leaves.** *"No caller" is an execution result, never a search
result.* Before declaring anything unmounted, dispatch it and watch it answer.
Pinned by `TestAMountedVerbIsProvenByExecution`, which awaits
`try_dispatch("/<verb> help")` for every verb declaring authored help rather
than asserting anything about source text.

**What the §28 audit had to learn from this.** Dynamic routing is an edge no
AST walker can see, so all 88 routed modules and their subtrees would read as
orphans. `surface_reachability._dispatch_mounted` supplies them as entry
points — detected **statically**, by matching the registry's own filename
convention over the AST it already parses. Asking the registry directly would
prime it, importing every verb module, and the audit's contract is that it
never imports the code it measures.
