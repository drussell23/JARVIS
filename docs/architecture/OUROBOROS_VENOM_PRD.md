...
**Phase 4 — Cognitive Metrics**: P3 in flight — Oracle pre-score + vindication wrapper now wired into the orchestrator
  ...
  - [x] P3 Slice 2 — orchestrator boot-time singleton wiring + CONTEXT_EXPANSION pre-score call site + 19 graduation pin tests + 15 in-process live-fire checks + dual env knob hot-revert (this PR). **`JARVIS_COGNITIVE_METRICS_ENABLED` default `false`→`true`.** Hot-revert: single env knob. Authority invariant: pre_score is **advisory only** — Iron Gate / risk_tier_floor remain authoritative; future slices can weight downstream decisions on the score. Layered evidence: 63 deterministic tests + 15 in-process smoke + end-to-end integration test (boot wire → CONTEXT_EXPANSION call → ledger row → REPL surfaces). **Phase 4 P3 COMPLETE — both stranded RSI modules now wired into the live FSM.**
  - [ ] Future Phase 4 work (vindication call site at post-APPLY, summary.json wiring, weighting cognitive scores in downstream gates) tracked separately.
