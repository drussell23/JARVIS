...
**Status update 2026-05-05**: 5 of 6 Wave 3 hygiene items shipped (Move 8 status reconciled + vector #11 monotonic migration + vector #9 sensitive-flag masking + §28.5.1 invariant_drift_store flock + vector #10 AutoCommitter race); 29/29 regression tests in `test_wave3_hygiene_2026_05_05.py` green. New `async_flock_critical_section` primitive added to `cross_process_jsonl.py` extends the §33.4 pattern with an async-safe variant — first reusable async lock primitive in the substrate. **Deferred**: vector #8 ArtifactContract drift (multi-hour schema-versioning arc — needs unified `ArtifactContract` base + per-artifact `schema_version` field + dual-reader pattern).
