...
| **Move 7 — Cross-op Semantic Budget** | §29.4 (line 3973) | 🟡 **Slice 1 SHIPPED 2026-05-05** — substrate primitive landed; Slices 2-5 deferred | Medium | Slice 1 (`cross_op_semantic_budget.py`, ~580 LOC) ships pure-function primitive: 5-value `SemanticBudgetVerdict` closed enum + frozen `OpSemanticCentroid` (adopts §33.5) + frozen `SemanticBudgetReport` + pure `compute_semantic_budget(centroids, *, threshold, ratio)` integrating cosine-distance deltas across rolling window. Master flag default-FALSE per §33.1 graduation contract pattern; 3 AST pins auto-discovered (master-flag-stays-default-false + authority-asymmetry + verdict-taxonomy-5-values). 4 FlagRegistry seeds (master + window_size + threshold + approaching_ratio). 40 regression tests. **Slices 2-5 deferred** (operator-paced empirical baseline first): Slice 2 centroid recorder at COMPLETE phase + flock'd JSONL ledger; Slice 3 async observer + SSE event + GET route; Slice 4 `/semantic-budget` REPL verb; Slice 5 graduation-contract harness gating master-flag flip on Phase-9-style 30+ op evidence ladder. |
