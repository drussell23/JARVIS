...
| 8 | M10 — `ArchitectureProposer` (refined per §32.4) | §30.5.2 (superseded by §32.4) | **NEXT** | Inherits 15-phase FSM + AdaptiveThreshold + H1–H6 + 5-layer validation from `graduation_orchestrator.py`. RSI safety gate now in place via Upgrade 2. |
| 8a | M10 — `ArchitectureProposer` (legacy entry; superseded by row 8 above) | §32.4 supersedes §30.5.2 | Pending | See row 8 above |
...
| 8 | M10 — ArchitectureProposer (refined per §32.4) | **IN PROGRESS** — Slice 1 done (52/52), Slices 2–5 pending |
| 9 | `graduation_orchestrator.py` cleanup arc | Pending (post-M10) |
...
**Recommended sequencing post-M10**:
1. Close M10 (Slices 2–5)
2. graduation_orchestrator.py cleanup arc (~1 day)
3. **Phase 10 Slices 2–6 BEFORE Phase 9** — cheaper soaks make Phase 9 affordable
4. Phase 9 — graduate the 12+ default-false flags via 3-clean-session soaks
5. Phase 6 self-narrative
6. Venom V1–V4 + GitHub Patterns B+C in parallel slots throughout
