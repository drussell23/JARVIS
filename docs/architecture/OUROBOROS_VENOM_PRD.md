...
**Phase 7 — Activation & Hardening** (NEW 2026-04-26 — converts Pass C from substrate to functional; per §3.6 brutal review): 🟡 IN-FLIGHT (6/9 slices landed — **5/5 Pass C activation surfaces + bounded HypothesisProbe primitive shipped**)
...
Next open phase items per the Forward-Looking Priority Roadmap: **Phase 7 continuation** (7.7 sandbox hardening + caller wiring for 7.1-7.5 loaders + Slice-6 YAML writer + HypothesisProbe production prober wiring + 7.8 cross-process flock + 7.9 stale-pattern sunset) → **Phase 6 P6** (Self-narrative, long-horizon, blocked on Phase 7 completion).
...
|---|---|---|---|---|
| 🔴 **#1** | **Phase 7.7 — Sandbox hardening** (AST-block `__subclasses__` / `__bases__` / `__class__` access) | 🔴 Not started | Closes the only known structural sandbox-escape vector. Operator-authorization is currently the SOLE defense after approval. | ~2 days |
| 🔴 **#2** | **Pass C activation pipeline caller wiring + per-loader cadence** (5 loaders to wire) | 🔴 Not started (5/5 loaders shipped 7.1-7.5) | Loaders exist but actual gate-state mutation on `/adapt approve` requires (a) Slice 6 writing approved proposals to `.jarvis/adapted_<surface>.yaml` and (b) callers (orchestrator / `SemanticGuardian._PATTERNS` / `ExplorationFloors` / `ScopedToolBackend` / `risk_tier_floor`) switching to consume the new helpers. | ~3 days wiring + Background cadence |
| 🔴 **#3** | **HypothesisProbe production wiring** (Phase 7.6 substrate complete) | 🔴 Not started (substrate landed) | `AnthropicVenomEvidenceProber` (read-only Venom subset honoring the allowlist) + bridge to `HypothesisLedger.record_outcome` + bridge to `AdaptationLedger.propose` for confirmed hypotheses. | ~3 days |
...
| 🟡 **#8** | **Pass C per-slice graduation cadence** (~10 flag flips across 6 slices) | 🔴 Not started | 5 clean sessions per slice (Pass C §4.6). Higher bar than Pass B. Starts after Phase 7 caller wiring lands. | Background |
| 🟢 **#9** | **Phase 6 P6 — Self-narrative** | 🔴 Not started | Now blocked by Phase 7 (needs *real* adaptation history, not just substrate). Long-horizon (3-6 months). | ~3 weeks (after Phase 7) |
| 🟢 **#10** | **Skills/workflows surface** (CC parity item) | 🔴 Not started | Nice-to-have UX. No PRD priority assigned. | Future |
| 🟢 **#11** | **Per-action permission granularity** (CC parity item) | 🔴 Not started | Tradeoff against autonomy; likely operator-rejected as anti-vision. | Possibly never |
...
**Phase 7 progress**: 6/9 slices landed — **all 5 Pass C activation surfaces have substrate-complete boot loaders + bounded HypothesisProbe primitive** (SemanticGuardian patterns / IronGate floors / per-Order budgets / risk-tier ladder / category weights / hypothesis-probe). ~6 days of remaining focused work for 7.7+7.8+7.9; 3 PRs.
