...
**Phase 7 — Activation & Hardening** (NEW 2026-04-26 — converts Pass C from substrate to functional; per §3.6 brutal review): 🟡 IN-FLIGHT (4/9 slices landed)
...
Next open phase items per the Forward-Looking Priority Roadmap: **Phase 7 continuation** (7.5 → 7.9, plus caller wiring + per-slice graduation cadence for the loaders shipped) → **Phase 6 P6** (Self-narrative, long-horizon, blocked on Phase 7 completion).
...
|---|---|---|---|---|
| 🔴 **#1** | **Phase 7.5 — Last Pass C activation surface** (category-weight rebalance) | 🔴 Not started (4/5 done — 7.1+7.2+7.3+7.4 landed) | Closes the last activation surface. Pass C goes from theater (1 surface) to fully-functional (5 surfaces). | ~1 day |
...
| 🟡 **#8** | **Phase 7.1+7.2+7.3+7.4 caller wiring + graduation cadence** | 🔴 Not started | Loaders shipped; the orchestrator/`SemanticGuardian._PATTERNS`/`ExplorationFloors`/`ScopedToolBackend`/`risk_tier_floor` callers must be switched to consume them, then 3-clean-session cadence per loader before flag flip. | ~3 days wiring + Background cadence |
...
**Phase 7 progress**: 4/9 slices landed (substrate + loader pattern proven across SemanticGuardian / IronGate / ScopedToolBackend / risk-tier ladder). ~10 days of remaining focused work; 5 PRs.
...
| **2** | **Pass C activation gap** — proposal pipeline shipped; 4/5 boot loaders now landed (SemanticGuardian / IronGate / per-Order budget / risk-tier ladder); category-weight rebalance + caller wiring + cadence remain | 🟡 Partially mitigated | Pass C is moving from observable theater to functional substrate. Loaders for the 4 highest-impact surfaces exist; only category-weight rebalance + callers + per-loader cadence remain | 🟡 In-flight via Phase 7 (4/9 slices done — 7.1+7.2+7.3+7.4 landed; 7.5 next) |
...
|---|---|---|---|---|
| 🔴 **#1** | **Pass C activation pipeline** (Phase 7) — wire `.jarvis/adapted_<surface>.yaml` consumption into all 5 gates' boot loaders | 🟡 4/5 loaders landed (7.1+7.2+7.3+7.4); 7.5 + caller wiring pending | **Highest**: closes the activation gap; transforms Pass C from theater → real | Four of the five activation surfaces have substrate-complete boot loaders. Caller wiring + the last surface (category-weight rebalance) close the gap end-to-end |
...
|---|---|---|
| Pass C activation gap (substrate vs functional) | 🟡 Partially mitigated (4/5 loaders shipped) | Phase 7 (#1 above) — finish 7.5 + caller wiring |
