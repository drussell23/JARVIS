...
| **Move 7 — Cross-op Semantic Budget** | §29.4 (line 3973) | 🟡 **Slices 1+2+3+4 SHIPPED 2026-05-05** — full operator surface complete; only Slice 5 (graduation contract) deferred | Medium | Slice 1: math primitive (40 tests). Slice 2: §33.2 producer-bridge + §33.4 flock'd JSONL (27 tests). Slice 3: async observer + SSE + auto-mounted GET route (27 tests, §33.3 naming-cage). **Slice 4 ships REPL surface**: `semantic_budget_repl.py` exposes `dispatch_semantic_budget_command(line) -> SemanticBudgetReplDispatchResult` with 4 subcommands (`status` / `recent [N]` / `window` / `help`); auto-discovered by §32.11 Slice 4 `repl_dispatch_registry` zero-edit (verb name `semantic_budget` derived from filename per naming-cage convention; auto-mount proven by `test_verb_auto_discovered_via_slice4_registry` end-to-end including `try_dispatch` routing); `register_verbs` /help auto-discovery hook present; `status` integrates Slices 1+2 over current window with verdict + drift_pct_of_budget projection; `recent N` projects ledger rows via Slice 2 reader; `window` renders env-knob configuration; help bypasses master gate; defensive shlex parse-error handling; 20 new tests + 2 new AST pins (authority-asymmetry + composes-substrate forbidding parallel math/ledger-read/flag-check); 441/441 across full sweep. Master flag default-FALSE preserved (§33.1). **Move 7 operator-facing surface NOW COMPLETE** — only Slice 5 graduation-contract harness remains (gates master-flag flip per §33.1 on accumulated empirical evidence; substrate-side ready, awaits Phase-9-style baseline). |
