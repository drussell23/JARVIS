...
**Phase 7 — Activation & Hardening** (NEW 2026-04-26 — converts Pass C from substrate to functional; per §3.6 brutal review): 🟡 IN-FLIGHT (3/9 slices landed)
  ...
  - [ ] Phase 7.4 — Risk-tier ladder activation (Pass C Slice 4b) — pending
  - [ ] Phase 7.5 — Category-weight rebalance activation (Pass C Slice 5) — pending
  - [ ] Phase 7.6 — Bounded hypothesis-probe loop — pending
  - [ ] Phase 7.7 — Sandbox hardening (AST validator Rule 7: reject `__subclasses__`/`__bases__`/`__class__` in candidate source) — pending
  - [ ] Phase 7.8 — Cross-process AdaptationLedger advisory locking (`fcntl.flock`) — pending
  - [ ] Phase 7.9 — Stale-pattern sunset signal — pending

Next open phase items per the Forward-Looking Priority Roadmap: **Phase 7 continuation** (7.4 → 7.9, plus caller wiring + per-slice graduation cadence for the loaders shipped) → **Phase 6 P6** (Self-narrative, long-horizon, blocked on Phase 7 completion).
...
|---|---|---|---|---|
| 🔴 **#1** | **Phase 7.4-7.5 — Remaining Pass C activation surfaces** (risk-tier ladder + category-weight rebalance) | 🔴 Not started (3/5 done — 7.1+7.2+7.3 landed) | Closes the last 2 of 5 activation surfaces. Pass C goes from theater (1 surface) to fully-functional (5 surfaces). | ~3 days |
| 🔴 **#2** | **Phase 7.6 — Bounded hypothesis-probe loop** | 🔴 Not started | Closes the autonomous-curiosity gap. Composes existing HypothesisLedger + PostmortemRecall + read-only Venom into a closing loop. Required for "scientific method as a loop". | ~1 week |
| 🔴 **#3** | **Phase 7.7 — Sandbox hardening** (AST-block `__subclasses__` / `__bases__` / `__class__` access) | 🔴 Not started | Closes the only known structural sandbox-escape vector. Operator-authorization is currently the SOLE defense after approval. | ~2 days |
| 🟡 **#4** | **Phase 7.8 — Cross-process AdaptationLedger advisory locking** | 🔴 Not started | Closes fragility vector #3. Safety-net for parallel agent-conducted soak runs. | ~1 day |
| 🟡 **#5** | **Phase 7.9 — Stale-pattern sunset signal** | 🔴 Not started | Closes fragility vector #4. Long-horizon adaptive-surface hygiene. Depends on 7.1 (built). | ~3 days |
| 🟡 **#6** | **Phase 8 — Temporal Observability** (5 sub-deliverables) | 🔴 Not started | Decision causal-trace + latent-confidence ring + multi-op timeline + flag-changed SSE + latency-SLO breach. Operator-UX-meaningful. | ~2 weeks (parallel to Phase 7) |
| 🟡 **#7** | **Pass B per-slice graduation cadence** (9 flag flips) | 🟡 In flight | Agent-conducted twice-daily soak (`trig_012EvEDkABy2u5PSSs3xK5C4`); ~14 days minimum to flip all 9. | Background |
| 🟡 **#8** | **Phase 7.1+7.2+7.3 caller wiring + graduation cadence** | 🔴 Not started | Loaders shipped; the orchestrator/`SemanticGuardian._PATTERNS`/`ExplorationFloors`/`ScopedToolBackend` callers must be switched to consume them, then 3-clean-session cadence per loader before flag flip. | ~2 days wiring + Background cadence |
| 🟡 **#9** | **Pass C per-slice graduation cadence** (~10 flag flips across 6 slices) | 🔴 Not started | 5 clean sessions per slice (Pass C §4.6). Higher bar than Pass B. Starts after Phase 7 caller wiring lands. | Background |
| 🟢 **#10** | **Phase 6 P6 — Self-narrative** | 🔴 Not started | Now blocked by Phase 7 (needs *real* adaptation history, not just substrate). Long-horizon (3-6 months). | ~3 weeks (after Phase 7) |
| 🟢 **#11** | **Skills/workflows surface** (CC parity item) | 🔴 Not started | Nice-to-have UX. No PRD priority assigned. | Future |
| 🟢 **#12** | **Per-action permission granularity** (CC parity item) | 🔴 Not started | Tradeoff against autonomy; likely operator-rejected as anti-vision. | Possibly never |

**Critical-path summary**: Phase 7 (#1-#5) **converts O+V from B− to A−**. Phase 8 (#6) makes the system debuggable at A-level. Phase 6 P6 lands AFTER Phase 7 because self-narrative needs real adaptive history.
...
| 7.4 | Risk-tier ladder activation | 🔴 Pending | Higher-risk (mutates canonical enum); ~2 days |
| 7.5 | Category-weight rebalance activation | 🔴 Pending | Composes with 7.2 module; ~1 day |
| 7.6 | Bounded hypothesis-probe loop | 🔴 Pending | Independent; ships parallel with 7.4-7.5; ~5 days |
| 7.7 | Sandbox hardening (AST validator Rule 7) | 🔴 Pending | Independent; extends Slice 3 validator; ~2 days |
| 7.8 | Cross-process AdaptationLedger flock | 🔴 Pending | Independent; ships anytime; ~1 day |
| 7.9 | Stale-pattern sunset signal | 🔴 Pending | Depends on 7.1 (built); ~3 days |

**Phase 7 progress**: 3/9 slices landed (substrate + loader pattern proven across SemanticGuardian / IronGate / ScopedToolBackend). ~2 weeks of remaining focused work; 6 PRs.

**Why this is still Priority 1**:
- **Phase 7 substrate is no longer theater for 3 surfaces** — boot-time loaders + cage-rule helpers exist for adapted SemanticGuardian patterns, IronGate floors, and per-Order mutation budgets. But the actual gate-state mutation on `/adapt approve` requires (a) writing the approved proposal to `.jarvis/adapted_<surface>.yaml` (Slice 6 follow-up) and (b) callers consuming the new helpers (substrate-first pattern means callers haven't switched yet).
- The novel architectural claim (per Pass A: "Anti-Venom adaptive thesis is genuinely novel") gains its first defensible footing once 7.1+7.2+7.3 callers wire through and a single `/adapt approve` cycle changes a live gate's behavior end-to-end.
...
|---|---|---|---|---|
| **1** | **Sandbox escape via object-graph traversal** in operator-approved candidates. `_SAFE_BUILTIN_NAMES` includes `object` + `type`; once approved, `object.__subclasses__()` walks to `subprocess.Popen` etc. | 🔴 Critical | The operator-authorization gate is currently the ONLY defense; the cage is "trust the operator" not "structural sandbox" | 🔴 Not mitigated. Either harden allowlist OR add SemanticGuardian pattern that AST-rejects `__subclasses__`/`__bases__` access in candidate source |
| **2** | **Pass C activation gap** — proposal pipeline shipped; 3/5 boot loaders now landed (SemanticGuardian / IronGate / per-Order budget); 2 surfaces remaining + caller wiring + cadence | 🟡 Partially mitigated | Pass C is moving from observable theater to functional substrate. Loaders for the highest-cited 3 surfaces exist; risk-tier ladder + category-weight rebalance still pending; callers haven't switched yet | 🟡 In-flight via Phase 7 (3/9 slices done — 7.1+7.2+7.3 landed; 7.4+7.5 next) |
| **3** | **Cross-process AdaptationLedger race** — `threading.RLock` + `os.fsync` only; no advisory file lock | 🟡 Medium | Two processes mining concurrently could write conflicting records; substrate dedupes at read-time but write path is race-prone | 🟡 Partial — current behavior is race-resistant by accident, not by construction. Add `fcntl.flock` advisory locking |
| **4** | **Semantic drift over long horizons** — mined SemanticGuardian patterns are additive forever; no sunset signal | 🟡 Medium | Stale patterns accumulate; operator must manually trigger Pass B `/order2 amend` to remove. No "this pattern hasn't matched in 30 days" signal | 🔴 Not started. Add stale-pattern detector + advisory sunset proposal |
...
|---|---|---|---|---|
| 🔴 **#1** | **Pass C activation pipeline** (Phase 7) — wire `.jarvis/adapted_<surface>.yaml` consumption into all 5 gates' boot loaders | 🟡 3/5 loaders landed (7.1+7.2+7.3); 7.4+7.5 + caller wiring pending | **Highest**: closes the activation gap; transforms Pass C from theater → real | Three of the five activation surfaces have substrate-complete boot loaders. Caller wiring + the last two surfaces (risk-tier ladder + category-weight rebalance) close the gap end-to-end |
| 🔴 **#2** | **Bounded hypothesis-probe loop** (Phase 7.6) — extend HypothesisLedger with read-only Venom probe + structural termination | Not started | **High**: closes the autonomous-curiosity gap §3.6.1 row "interleaved reasoning" surfaced | The "Curiosity Primitive" the operator describes exists in pieces; this composes them |
| 🔴 **#3** | **Sandbox hardening OR explicit accepted-risk documentation** (Phase 7.7) — close vector #1 above | Not started | **High**: removes the only known structural escape vector; or pins it as accepted with operator-authorization as the boundary | Honest choice: AST-validator addition that rejects `__subclasses__`/`__bases__`/`__class__` access in candidate source. ~2 days work |
...
| 🟡 #5 | **Cross-process AdaptationLedger advisory locking** — fix vector #3 | Not started | Medium: safety-net for parallel agent-conducted soaks | `fcntl.flock` add. ~1 day |
| 🟡 #6 | **Stale-pattern sunset signal** — fix vector #4 | Not started | Medium: prevents long-horizon adaptive-surface bloat | Generates `/adapt sunset-candidate` signals after 30d-no-match |
...
|---|---|---|
| Pass C activation gap (substrate vs functional) | 🟡 Partially mitigated (3/5 loaders shipped) | Phase 7 (#1 above) — finish 7.4 + 7.5 + caller wiring |
| One verified multi-file APPLY landmark | 🔴 Critical | Sustained soak + new landmark beyond Sessions Q-S |
| Sandbox object-graph escape vector | 🔴 Critical | Phase 7 (#3 above) |
| Cross-process race (vector #3) | 🟡 Medium | Phase 7 (#5 above) |
| Long-horizon stale-pattern accumulation | 🟡 Medium | Phase 7 (#6 above) |
| No bounded hypothesis-test loop | 🟡 Medium | Phase 7 (#2 above) |
