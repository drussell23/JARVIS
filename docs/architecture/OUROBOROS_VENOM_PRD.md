...
### Where we stand (2026-05-05 — post-Move-7+8 closures + Wave 3 hygiene + §28.5.1 closure + §33 meta-pattern catalog)

**Refresh marker (2026-05-05)**: this section was originally written 2026-04-29; what follows below remains historically correct as of that date. The May 5 reality (post §1 refresh) is captured in §36 — see "Where we stand v10" for the up-to-date one-page picture. Critical deltas since 2026-04-29:

- **Move 7 Cross-op Semantic Budget FULLY CLOSED 2026-05-05** (5 slices end-to-end same-day): math primitive + producer-bridge JSONL ledger + posture-aware async observer + chatter-suppressed SSE + auto-mounted GET route + REPL verb + §33.1 graduation contract harness. Closes the §29.4 v3 Move 7 framing — RSI drift now bounded mathematically in BOTH axes (Move 4 architectural-promise drift + Move 7 semantic-meaning drift). 470/470 regression tests across the full sweep.
- **Move 8 Proactive Curiosity Loop FULLY CLOSED 2026-05-05** (3 slices end-to-end same-day): closes the auto-spawn-exploration-ops half of M9's producer-consumer loop. `proactive_curiosity_reader.py` reader + ProactiveExplorationSensor wire-up (3rd signal source — composes existing `scan_once`, no parallel poll loop) + §33.1 graduation contract. RSI exploration-axis closed structurally; substrate-ready, master flag stays default-FALSE pending operator-paced empirical cadence.
- **Wave 3 hygiene arc FULLY CLOSED 2026-05-05** (6 of 6 items shipped same-day): vector #11 monotonic migration + vector #9 sensitive-flag masking + vector #10 AutoCommitter race + vector #8 Versioned-Artifact-Contract + §28.5.1 invariant_drift_store flock + Move 8 status reconciliation. Two new substrate primitives crystallized: `cross_process_jsonl.async_flock_critical_section` (first async-safe cross-process lock) + `meta.versioned_artifact` (canonical schema-versioning contract — §33.5 pattern).
- **§28.5.1 4-phases-not-extracted hygiene CLOSED 2026-05-05**: investigation revealed all 11 phases ALREADY extracted via `phase_runners/` package — brutal-review entry was stale. Closure was documentation-update + structural pin (`test_phase_runner_extraction_closure.py` 31/31 green) matching Move 8 Wave 3 Item 1 reconciliation pattern.
- **§33 Reusable Meta-Patterns catalog crystallized 2026-05-05**: 5 disciplines documented (Graduation Contract / Producer-Bridge / Slice 5b Naming-Convention Cage / Per-Cluster flock'd JSONL Persistence / Versioned-Artifact-Contract). Future arcs inherit by reference; pattern reuse rate has crossed the threshold where the PRD carries the load — new arcs are 30-40% smaller because §33 patterns compose instead of being re-discovered.
- **§35 Open Strategic Moves Registry**: every in-session-executable open item now closed. Remaining items are operator-paced empirical (Phase 9 cadence — closes 🔴 Critical vector #6 + provides empirical baseline for Move 7 + Move 8 graduation calibration) OR long-horizon scoping (M12 JPrimeLoRA, vector #12 Provider chain SPOF).

**Net empirical grade movement** since 2026-04-29:
- Architecture A → **A** (unchanged ceiling; load increased without regressing structure)
- Cognitive depth A → **A** (Move 7 + Move 8 close the two RSI bounding axes structurally)
- Self-tightening immunity A → **A+** (16 new AST pins + 5 §33 patterns crystallized = immune system grew faster than the shell)
- RSI scaffolding gear count holds at 6/6 verified, **Gear 4 (Cross-op semantic stability) substrate ready** post-Move-7 — pending operator-paced graduation cadence
...
| **Long-horizon semantic stability** | 🟢 A− | A | **TrajectoryAuditor un-stranded 2026-05-04** — `observability/trajectory_auditor_observer.py` boots via `governed_loop_service.py` (boot snapshot + 6h periodic tick + SSE on `drifting`/`alarming` verdicts only). Master flag `JARVIS_TRAJECTORY_AUDITOR_OBSERVER_ENABLED` graduated default-true. `EVENT_TYPE_TRAJECTORY_DRIFT_DETECTED` published on warning/critical drift. The codebase walk runs in `asyncio.to_thread` so boot is non-blocking. Per-trajectory drift detection is now LIVE in production. 16/16 tests green. |
...
- [x] **Operation Modes (`plan/analyze/apply/auto`)** *(GitHub recon Pattern B)* — ✅ **SHIPPED 2026-05-07** (`operation_mode.py` + `mode_repl.py` + `tool_executor.py:_maybe_block_by_operation_mode` wired BEFORE V2 permission check; 51 regression tests; `JARVIS_OPERATION_MODE_ENABLED` default-FALSE per §33.1; closed 4-value `OperationMode` enum; `_MUTATION_TOOLS` reused via composition; ContextVar bridge for async-safe session state; `/mode` REPL verb auto-discovered).
- [x] **Component Tool Scope (per-sensor / per-subagent `allowed_tools`)** *(GitHub recon Pattern C)* — ✅ **SHIPPED 2026-05-07** (`component_tool_scope.py` + `scope_repl.py` + `tool_executor.py:_maybe_block_by_component_scope` wired AFTER OperationMode + BEFORE V2; 57 regression tests; `JARVIS_COMPONENT_TOOL_SCOPE_ENABLED` default-FALSE per §33.1; closed 4-value `ComponentScopeDecision` enum; V4 `tool_name_pattern` matcher composed via memoized `_compile_pattern_cached` — no parallel regex math; ContextVar bridge for async-safe component identity propagation; per-component allowlist + denylist with deny-wins-over-allow; `/scope` REPL verb auto-discovered with show/check/active subcommands).
...
#### P10.2 — YAML v2 schema + dual-reader *(SHIPPED — audited 2026-05-05)*

**Status**: ✅ MERGED. `provider_topology.py:85` defines `SCHEMA_VERSION_V2 = "topology.2"`; `RouteEntryV2` frozen dataclass + `Topology.from_v2()` classmethod + dual-reader (v2 first, v1 fallback) are all live; `brain_selection_policy.yaml:343` is on `topology.2`. Audit revealed Slice 2 had landed earlier under Phase 12 Slice E coordination — the PRD §32.8.1 v4 supplement (added 2026-05-04) was authored before this discovery and is corrected here.

**Problem (historical)**: yaml v1 only allowed ONE `dw_model` per caller and forced a single `dw_allowed: bool` per route. There was no way to express "try Kimi-K2.6 first, then GLM-5.1, then Qwen3.6-35B before cascading."
...
#### P10.3 — `candidate_generator.py` consumer wiring *(SHIPPED — audited 2026-05-05)*

**Status**: ✅ MERGED. `candidate_generator.py:1703-1762` carries the AsyncTopologySentinel gate behind `JARVIS_TOPOLOGY_SENTINEL_ENABLED`. `preflight_check()` raises `SentinelInitializationError` at the gate so boundary-isolation defects surface at the point of decision, not minutes later in the postmortem (closes the silent-degradation regression that bit `bt-2026-04-27-194550`). `_dispatch_via_sentinel(context, deadline, route)` walks the route's ranked `dw_models` list trying each model whose breaker isn't OPEN; on full-route SEVERED applies `fallback_tolerance` (`cascade_to_claude` invokes `_call_fallback`; `queue` raises `dw_severed_queued:<route>:<models>:<reason>`). BG/SPEC `queue` contract pinned at routing layer per `project_bg_spec_sealed.md`.

**Problem (historical)**: the static topology gate at `candidate_generator.py:1404-1465` read yaml v1 directly; sentinel state was invisible to the routing decision.
...
#### P10.4 — Live-exception failure ingest *(SHIPPED — audited 2026-05-05)*

**Status**: ✅ MERGED. `candidate_generator.py` wires `sentinel.report_failure(model_id, FailureSource.LIVE_STREAM_STALL, detail)` at FOUR DW failure sites (lines 2482, 2494, 2514, 2524 — exceeded the PRD-anticipated 3). Live-exception weight is 3.0 by default — single occurrence trips the breaker immediately, faster than probe-only detection. Backward-compatible legacy `report_failure` adapter handled at line 2509-2530.

**Problem (historical)**: probe-only failure detection meant a real GENERATE op stream-stall observed by the orchestrator never reached the sentinel — the next op repeated the same DW attempt before the next probe ran.
...
| ~~15~~ | ~~Per-tool permissions (Venom V2)~~ | §32.6 V2 | ~~~2 slices~~ | ✅ **SHIPPED 2026-05-07** (V1+V2+V3+V4 all closed same-day; 595/595 cumulative regression green; 4 graduation contracts per §33.1; `PermissionRegistration.is_async` AST-pinned absent — V3 is V1-hook-path-only per operator binding 2026-05-07) |
