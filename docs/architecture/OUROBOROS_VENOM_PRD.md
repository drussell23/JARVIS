...
Next open phase items per the Forward-Looking Priority Roadmap: **Phase 7 continuation** (caller wiring for 7.1-7.5 loaders + Slice-6 YAML writer + 7.6 hypothesis-probe loop + 7.7 sandbox hardening + 7.8 cross-process flock + 7.9 stale-pattern sunset) → **Phase 6 P6** (Self-narrative, long-horizon, blocked on Phase 7 completion).
...
|---|---|---|---|---|
| 🔴 **#1** | **Phase 7.6 — Bounded hypothesis-probe loop** | 🔴 Not started | Closes the autonomous-curiosity gap. Composes existing HypothesisLedger + PostmortemRecall + read-only Venom into a closing loop. Required for "scientific method as a loop". | ~1 week |
| 🔴 **#2** | **Phase 7.7 — Sandbox hardening** (AST-block `__subclasses__` / `__bases__` / `__class__` access) | 🔴 Not started | Closes the only known structural sandbox-escape vector. Operator-authorization is currently the SOLE defense after approval. | ~2 days |
| 🔴 **#3** | **Pass C activation pipeline caller wiring + per-loader cadence** (5 loaders to wire) | 🔴 Not started (5/5 loaders shipped 7.1-7.5) | Loaders exist but actual gate-state mutation on `/adapt approve` requires (a) Slice 6 writing approved proposals to `.jarvis/adapted_<surface>.yaml` and (b) callers (orchestrator / `SemanticGuardian._PATTERNS` / `ExplorationFloors` / `ScopedToolBackend` / `risk_tier_floor`) switching to consume the new helpers. | ~3 days wiring + Background cadence |
...
**Phase 7 progress**: 5/9 slices landed — **all 5 Pass C activation surfaces now have substrate-complete boot loaders** (SemanticGuardian patterns / IronGate floors / per-Order budgets / risk-tier ladder / category weights). ~10 days of remaining focused work for 7.6+7.7+7.8+7.9; 4 PRs.
