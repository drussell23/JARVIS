...
|---|---|---|---|---|
| 10 | `--rerun-from <session>:<phase>` + `/replay` REPL | §36.4 Priority #2 | ~3d (audit revealed: thin wrapper around existing primitives, ~150 LOC single-slice viable) | 🚧 **IN-PROGRESS 2026-05-05** — audit complete; `decision_runtime.py` + `causality_dag.py` + `replay_from_record.py` substrate already exists; gap is `<session>:<phase>` parsing + DAG phase-filter + ordinal-reset helper |
