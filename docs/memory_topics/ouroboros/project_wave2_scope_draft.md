...
| 4 | **VALIDATE** + **GATE** + **APPROVE** + **APPLY** + **VERIFY** | ~2,500 lines total | Mid-size phases. VALIDATE and GATE have nested retry loops — extract faithfully. **Delivered as 4a.1 (VALIDATE) → 4a.2 (GATE) → 4b (APPROVE+APPLY+VERIFY)** — split authorized 2026-04-21 to keep per-sub-slice parity depth (nested-retry FSM, shadow harness, 5a/5b previews, SemanticGuardian, MutationGate, risk floor) honest rather than thinned to fit context. |
| 5 | **GENERATE** | 1,926 lines | The beast. **Delivered as 5a → 5b** — split authorized 2026-04-22. 5a (spine): prelude + retry skeleton + CandidateGenerator dispatch + cost cap + happy/bounded-retry/L2-escape terminals. 5b (§6 depth): Iron Gate suite parity — exploration ledger category-aware path, ASCII strict, dependency-file integrity, multi-file coverage, retry-feedback composition. **5b owns §6 coverage.** Same runner module + flag across both sub-slices. |
| 6 | Dispatcher cleanup | ~100 lines | Replace orchestrator's inline `ctx.advance(...)` calls with a loop over a PhaseRunner registry. **Delivered as 6a → 6b → post-graduation deletion** (split authorized 2026-04-22). 6a: `PhaseRunnerRegistry` + `PhaseContext` + dispatcher loop + `JARVIS_PHASE_RUNNER_DISPATCHER_ENABLED` flag + infrastructure parity (registry miss / unknown phase / malformed runner/result / iteration cap / happy + cancel terminal parity dispatcher-on ≡ dispatcher-off). 6b: deep per-phase terminal parity matrix + artifact propagation edge cases. **Post-graduation (separate maintenance PRs):** delete inline `else:` blocks once individual phase flags graduate — dispatcher becomes canonical. |
