...
4. **Minimum-viable fallback guard** — `_call_fallback` skips the API call when remaining budget is below the minimum-viable floor, raising immediately with a diagnostic message instead of a doomed timeout. Since 2026-08-01 that floor is `_min_viable_fallback_s()`, which delegates to `admission_gate.min_viable_call_s()` (`JARVIS_ADMISSION_MIN_VIABLE_CALL_S`, default **25s**, clamped [10, 60]). It previously read its own `OUROBOROS_MIN_VIABLE_FALLBACK_S` (default 10s) — a second answer to the same question that the admission gate always overrode at default settings, and that came back to life whenever the gate was disabled. The legacy variable is now ignored, with a startup warning if set.
