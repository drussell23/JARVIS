#!/usr/bin/env bash
# Sanctioned diversity harvest -- soak 18: the CLEAN-SAMPLE run.
#
# Env is soak 17 byte-for-byte. What changed is code, default-on:
#   LINEAGE PURIFICATION (jarvis + reactor). Soak 17 paired "twins" at
#   1.0000 because L2 repair re-generations were recorded as sibling
#   attempts of the draw they repaired (29 of 43 rows). Every row now
#   carries metadata.draw_kind (primary | sibling | repair | retry), a
#   lineage guard prunes repair/retry rows that re-record an earlier hash,
#   persistence dedupes by (op_id, candidate_hash), and both the harvest
#   and Reactor ingestion pair only genuine primary/sibling draws.
#
# The question, on a sample that is finally what it claims to be:
#   does the widened reward geometry clear --min-spread 0.01?
#
# Proof lines, in order:
#   corpus rows carry "draw_kind"                            discriminator live
#   "[TrajectoryRecorder] pruned ... draw"                   guard fired (if L2 ran)
#   harvest rows_repair / rows_deduped                       filter live
