...
- **2026-04-20 (Ticket #4 Slice 3 — TestRunner streaming, default off)** — TestRunner's new `_exec_with_streaming` path consumes `BackgroundMonitor` DIRECTLY (not via Venom — infra stays deterministic). Env knobs all default false: `JARVIS_TEST_RUNNER_{STREAMING_ENABLED,EARLY_EXIT_ON_FAIL,PARITY_MODE}`. Streaming uses pytest `-v`; legacy `-q` unchanged. Structural TestResult parity (`passed`/`total`/`failed`/`failed_tests`) enforced by integration tests running BOTH paths on passing + mixed fixtures — FAIL LOUDLY on divergence. Optional `event_callback` ctor kwarg (default `None`, exceptions caught). Grep-stable INFO contract: `[TestRunner] streaming test_{passed,failed,errored,skipped} node=X sequence=N`. Parity mode logs `parity_ok`/`parity_divergence`. Isolation pinned: 2 tests enforce test_runner.py imports background_monitor, NOT monitor_tool. **18 Slice 3 tests green; combined Ticket #4: 92/92**. Slice 4 owns graduation. Manifesto §1+§8. Details: `memory/project_ticket_4_testrunner_streaming.md`.
- **2026-04-20 (Ticket #4 Slice 2 — Venom `monitor` tool, deny-by-default)** — Policy-gated read-only observability tool wrapping Slice 1's BackgroundMonitor. New `backend/core/ouroboros/governance/monitor_tool.py` (~280 lines) provides `run_monitor_tool(call, policy_ctx, timeout, cap) → ToolResult` with structured JSON output (`exit_code`, `duration_s`, `events[]`, `early_exit`, `timed_out`, `truncated`). Manifest registered in `_L1_MANIFESTS` with capabilities `{"subprocess"}` (NO `"write"` — read-only category, NOT in `_MUTATION_TOOLS`, allowed under `is_read_only` scope). Dispatch branch added to `AsyncProcessToolBackend._run_async_native_tool`. **Policy Rule 16** added to `GoverningToolPolicy.evaluate`: deny-by-default (`JARVIS_TOOL_MONITOR_ENABLED` defaults `false` — opposite of run_tests/bash which default `true`), binary allowlist (`JARVIS_TOOL_MONITOR_ALLOWED_BINARIES` CSV — default `"pytest,python,python3,node,npm,go,cargo,make,ruff,mypy,pyright"`), structural arg validation. Handler caps effective timeout at `min(model_requested, JARVIS_TOOL_MONITOR_TIMEOUT_S ceiling, Venom deadline)` — model cannot exceed operator ceiling. Output events capped at `JARVIS_TOOL_MONITOR_MAX_EVENTS` (default 500). Defense-in-depth: handler re-validates args even when policy approves, catches FileNotFoundError (binary vanished race), bad regex, and all uncaught exceptions at the tool boundary. **30/30 regression tests green** covering manifest + authority (3: registered, not-in-_MUTATION_TOOLS, allowed-under-read-only-scope), policy deny/allow matrix (9: master-off / explicit-false / binary-not-allowed / bad-args × 3 / happy-path / basename-match / empty-allowlist-denies), handler behavior (8: happy path, pattern early-exit, timeout, env-ceiling cap, binary-not-found, bad-regex, missing-cmd, max-events-cap), observer invariant preserved (1: completes without event_bus), helper-function pins (9). **What Slice 3 consumes**: TestRunner migration imports `BackgroundMonitor` PRIMITIVE directly (not this Venom tool) — TestRunner is infra without a PolicyContext and doesn't need the deny/allow gate. Manifesto §1 (Boundary Principle — tool surface does not grant new execution authority) + §8 (Absolute Observability — subprocess stream visible without mutating audit trail). Details: `memory/project_ticket_4_monitor_tool.md`.
- **2026-04-20 (Ticket #4 Slice 1 — BackgroundMonitor primitive)** — Closes the CC-parity observability gap: Claude Code streams stdout events live, O+V polled blocking subprocess.run. New `backend/core/ouroboros/governance/background_monitor.py` provides async context manager wrapping `asyncio.create_subprocess_exec` (argv-only, no shell) with line-granular `MonitorEvent` streaming (`stdout`/`stderr`/`exited`/`error` kinds) sharing a monotonic sequence counter across streams, bounded ring buffer (FIFO eviction via `deque(maxlen=N)`, immutable `Tuple` snapshot), optional `TrinityEventBus` publish (`{prefix}.{op_id}.{kind}` topics, `persist=False` for ephemeral stream events, exceptions caught + DEBUG-logged so bus misconfig never kills the monitor), backpressure via blocking `put()` (stalls subprocess stdio rather than dropping events), graceful SIGTERM→grace→SIGKILL shutdown in `__aexit__` (populates `exit_code` before cancelling reader tasks — prevents None on mid-cancel). Ordering guarantee: `KIND_EXITED` is always the LAST event yielded (`_await_exit` waits for stdout/stderr readers to drain via `asyncio.gather` before reaping + emitting terminal event). Non-UTF8 safety via `decode(errors="replace")`. Long-line safety (`LimitOverrunError` 64KB cap) via `truncated=True` partial-buffer events. Authority posture: observability only — Slice 1 ships NO tool surface / NO TestRunner migration. **21/21 regression tests green** covering spawn+teardown, event shape, stream mixing + ordering pin, ring bounds + immutability, non-UTF8, graceful shutdown (SIGTERM escalation), bus publish/None/exception, concurrent monitors (no cross-contamination), input validation, argv-not-shell security pin. Manifesto §8 (Absolute Observability) substrate for upcoming Slice 2 (Venom Monitor tool) + Slice 3 (TestRunner migration behind `JARVIS_TEST_RUNNER_STREAMING_ENABLED`) + Slice 4 (graduation). Details: `memory/project_ticket_4_background_monitor.md`.
