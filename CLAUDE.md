...
- **IDEObservabilityRouter** (`ide_observability.py`, Gap #6 Slice 1): Read-only HTTP GET surface for IDE extensions mounted beside webhook routes on `EventChannelServer`. Three endpoints: `GET /observability/health`, `GET /observability/tasks` (op_ids with TaskBoards), `GET /observability/tasks/{op_id}` (bounded projection `{task_id,state,title,body,sequence,cancel_reason}`). **Deny-by-default** (`JARVIS_IDE_OBSERVABILITY_ENABLED` default `false`) — disabled returns 403 (port scanners see no signal). **Loopback-only**: `assert_loopback_only()` accepts `{127.0.0.1, ::1, localhost}`, rejects `0.0.0.0`/`::`/wildcard before router mount. **Rate-limited**: sliding-window 120/min per client IP (`…_RATE_LIMIT_PER_MIN`). **CORS**: regex allowlist (`…_CORS_ORIGINS`, default matches localhost), echoes matched origin only — no `*`, no `Access-Control-Allow-Credentials`. Every response stamped `schema_version: "1.0"` + `Cache-Control: no-store`. Op_id regex `^[A-Za-z0-9_\-]{1,128}$` — malformed → 400. **Authority invariant** (grep-enforced by test): module never imports orchestrator/policy/iron_gate/risk_tier/gate modules — observability answers *"what is the loop doing"* never *"what should the loop do"*. Manifesto §1 Boundary Principle. Regression spine: `tests/governance/test_ide_observability.py` (31 tests).
- **IDEStreamRouter + StreamEventBroker** (`ide_observability_stream.py`, Gap #6 Slice 2): SSE stream at `GET /observability/stream` for live push of TaskBoard transitions. **SSE over WebSocket** — unidirectional transport = no covert command surface. 10-type event vocabulary (`task_{created,started,updated,completed,cancelled}` / `board_closed` / `heartbeat` / `stream_lag` / `replay_{start,end}`). **Deny-by-default** (`JARVIS_IDE_STREAM_ENABLED` default `false`). Bounded broker: 8 subscribers × 64-slot queue × 512-event history × 15s heartbeat × 10/min subscribe rate. Drop-oldest backpressure emits a single `stream_lag` control frame per lag-window. `Last-Event-ID` triggers history replay with `replay_{start,end}` markers (`known=false` when ack ID was evicted). Optional `?op_id=X` filter; control frames bypass it. Singleton broker via `get_default_broker()`; task_tool handlers publish post-transition via best-effort `_publish_stream_event()` (never raises; silent no-op when flag off); `close_task_board()` publishes `board_closed` before registry eviction. Reuses Slice 1 `assert_loopback_only()` + CORS allowlist. **Authority invariant** (grep-enforced): never imports orchestrator/policy/iron_gate/risk_tier/semantic_{guardian,firewall}. Regression: `tests/governance/test_ide_observability_stream.py` (41 tests).
...
- **2026-04-20 (Gap #5 — CLOSED, 4-slice arc ref `1b41725375` + graduation)** — "Structured to-do lists / what am I working on right now" view. Slice 4 flipped `JARVIS_TOOL_TASK_BOARD_ENABLED` default `false`→`true`; prompt injection flag already default-on by Slice 3 design. 17 graduation pins (single-flag revert matrix + authority invariants + structural safeguards + orchestrator-wiring preservation + docstring bit-rot). Live-fire proof `livefire-gap5-1776743088` empirically verified graduated defaults end-to-end: 6 Venom tool calls (2× create, update-start, update-edit, complete, update-cancel) all ALLOWED + SUCCESS with zero env overrides; 8 [TaskBoard]/[TaskTool] audit lines captured; `close_task_board(ctx.op_id, reason=...)` shutdown hook cleans up registry. Manifesto §1+§6+§8 preserved through graduation — manifest caps still empty, tools still NOT in _MUTATION_TOOLS, module doesn't import gate modules, per-transition INFO logs still the authoritative audit. **107/107 combined tests green** (33 Slice 1 + 35 Slice 2 + 22 Slice 3 + 17 Slice 4). Details: `memory/project_gap_5_{taskboard_slice1,task_tool_slice2,slice3_prompt_injection,slice4_graduation}.md`.
