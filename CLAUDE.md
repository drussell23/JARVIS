...
- **2026-04-20 (Phase C Epoch 3 Slices 3a+3b+3c — Semantic Index v1.0)** — Synthetic Soul (§4) expansion after cage sealed. Slice 3a: hand-rolled NumPy k-means + auto-K silhouette + cluster-kind classifier (`goal`/`conversation`/`postmortem`/`mixed`) + shadow-mode alignment + failure-gravity tripwire (rolling 50-signal window, WARN + counter). Slice 3c: themed `format_prompt_sections()` renders `### Theme: <label> (N items, <kind>)` blocks with deterministic tokenizer labels; K=1 → v0.1 fallback. Slice 3b: `JARVIS_SEMANTIC_CLUSTER_SCORING_POLICY={"centroid"|"max_cluster"}` (default `centroid`). Under `max_cluster`: `score()` returns max cosine across clusters; `boost_for()` applies **zero-boost-with-evidence** for postmortem-kind winners (boost zeroed, alignment still observed in histogram + failure-gravity + evidence). New stats: `scoring_policy`, `postmortem_boost_suppressions`, `scored_by_policy`. `score_with_cluster()` extended with `policy_used` + `boost_applied`. Defensive fallback: `max_cluster` with empty clusters degrades to `centroid`. Authority invariant preserved (no new consumer surface). **118/118 tests green**. Next: 3d two-flag graduation (CLUSTER_MODE + SCORING_POLICY both → active defaults). Details: `memory/project_phase_c_semantic_index_v1.md`.
