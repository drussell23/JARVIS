...
- **IDEObservabilityRouter** (`ide_observability.py`, Gap #6 Slice 1): Read-only GET endpoints for IDE extensions on `EventChannelServer`: `/observability/health`, `/observability/tasks`, `/observability/tasks/{op_id}` (bounded projection `{task_id,state,title,body,sequence,cancel_reason}`). Deny-by-default `JARVIS_IDE_OBSERVABILITY_ENABLED` (disabled → 403). Loopback-only assert (`{127.0.0.1,::1,localhost}` only). Rate-limited 120/min per client IP. CORS regex allowlist (no `*`, no credentials). Every response stamped `schema_version: "1.0"` + `Cache-Control: no-store`. Op_id regex `^[A-Za-z0-9_\-]{1,128}$`. Authority invariant (grep-enforced): never imports orchestrator/policy/iron_gate/risk_tier/gate modules. Manifesto §1 Boundary. Regression: `tests/governance/test_ide_observability.py` (31 tests).
- **IDEStreamRouter + StreamEventBroker** (`ide_observability_stream.py`, Gap #6 Slice 2): SSE at `GET /observability/stream`. Unidirectional → no covert command surface. 10 event types (`task_{created,started,updated,completed,cancelled}` / `board_closed` / `heartbeat` / `stream_lag` / `replay_{start,end}`). Deny-by-default `JARVIS_IDE_STREAM_ENABLED`. Bounded: 8 subs × 64 queue × 512 history × 15s heartbeat × 10/min rate. Drop-oldest emits single `stream_lag` per lag-window. `Last-Event-ID` replay + `replay_{start,end}` markers (`known=false` on eviction). `?op_id=X` filter; control frames bypass. task_tool hooks publish post-transition; `close_task_board()` publishes `board_closed`. Reuses Slice 1 loopback + CORS. Authority invariant grep-enforced. Regression: `tests/governance/test_ide_observability_stream.py` (41 tests).
- **VS Code extension** (`extensions/vscode-jarvis/`, Gap #6 Slice 3): Read-only TypeScript consumer of Slice 1 GET + Slice 2 SSE. Tree View + Op Detail webview (CSP-hardened, no scripts) + status bar + command palette + OutputChannel. Extension never POSTs back; authority invariant mirrors server-side §1. `ObservabilityClient` (fetch wrapper w/ AbortSignal + schema validation). `StreamConsumer` (native `fetch`+`ReadableStream` SSE parser, no EventSource dep; exponential-backoff+jitter reconnect bounded by `reconnectMaxBackoffMs`; `Last-Event-ID` on reconnect; signal-driven `reader.cancel()` for prompt stop). `OpsTreeProvider` applies stream events to bounded LRU (`maxOpsCached` default 256); `stream_lag` triggers hard-refresh. Poll fallback on error/reconnecting. Settings: `endpoint`/`enabled`/`autoReconnect`/`reconnectMaxBackoffMs`/`pollIntervalMs`/`opIdFilter`/`maxOpsCached`. Strict TS, Node 18+ `node --test` harness, 35 tests green. Build: `npm install && npm run compile` → `dist/extension.js`.
