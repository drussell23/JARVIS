...
- **2026-04-20 (Gap #5 Slice 2 — Venom task tools, deny-by-default)** — Three tools (`task_create`/`task_update`/`task_complete`) wrapping Slice 1's TaskBoard primitive. New `backend/core/ouroboros/governance/task_tool.py`; manifests + dispatch + **Rule 17** added to `GoverningToolPolicy.evaluate`. Deny-by-default via `JARVIS_TOOL_TASK_BOARD_ENABLED` (default false — mirrors Ticket #4 Slice 2 discipline). All three manifests carry `capabilities=frozenset()` (empty — no side effects on repo/subprocess/network); NOT in `_MUTATION_TOOLS`; allowed under `is_read_only` scope. Tight per-tool schemas: `task_create` takes title+body; `task_update` has mutually-exclusive shapes (state transition via `action` ∈ {start,cancel} OR content edit via title/body); `task_complete` takes task_id. Shared `classify_task_args` validator called from both policy layer + handler (defense-in-depth). Per-op registry `_BOARDS: Dict[op_id, TaskBoard]` with lazy-create on first touch; `close_task_board(op_id, reason)` is the single canonical shutdown API (idempotent). Handler routes to `TaskBoard` APIs only — no parallel state. Every failure → `EXEC_ERROR` with deterministic prefix (`state:`/`capacity:`/`board_closed:`). **35/35 Slice 2 tests** + 33/33 Slice 1 = **68/68 combined green**. Grep-enforced invariants: module doesn't import iron_gate/risk_tier_floor/semantic_guardian; module imports `task_board` primitive. Scope freeze honored: no orchestrator ctx.close_task_board() call-site yet (Slice 3), no prompt injection (Slice 3), no default flip (Slice 4). Manifesto §1 (tasks never become execution authority) + §6 (no Iron Gate / merge / validator branches on task state). Details: `memory/project_gap_5_task_tool_slice2.md`.
