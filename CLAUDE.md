...
- **2026-04-20 → 2026-04-21 (Gap #6 — CLOSED, 7-slice arc)** — IDE integration surface. Slices 1/2/4 shipped the server (GET + SSE + graduation); Slices 3/5/6 shipped three IDE clients (VS Code/Cursor, Sublime Text, JetBrains); Slice 7 proved the full stack end-to-end. **Server**: `/observability/{health,tasks,tasks/{op_id}}` + SSE `/observability/stream` with 10-event vocabulary, bounded broker (8×64×512), drop-oldest + `Last-Event-ID` replay + heartbeat, loopback-only + rate-limit + CORS allowlist, grep-enforced authority-import ban. Both env flags graduated default `true` on 2026-04-20 with 18 graduation pins. **Clients** (all read-only, schema_version "1.0"): (a) `extensions/vscode-jarvis/` TypeScript (35 tests, Cursor zero-change compat); (b) `extensions/sublime-jarvis/` pure-stdlib Python 3.8 plugin (27 tests); (c) `extensions/jetbrains-jarvis/` Kotlin + Gradle + IntelliJ Platform SDK, ships to every JetBrains IDE (28 Kotlin tests). **Slice 7 live-fire** (`scripts/livefire_gap6.py`) boots the server, subscribes a raw-socket SSE reader, emits 4 TaskBoard transitions, asserts all 4 reach the client as schema-v1.0 frames — journal at `.livefire/gap6-<ts>/journal.json`. **Total: 125 Python + 35 TypeScript + 27 Sublime + 28 Kotlin + 1 live-fire = 216 governance/client checks**. Details: `memory/project_gap_6_*.md`.
