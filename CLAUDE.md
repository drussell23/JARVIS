...
```
JARVIS (Body) <--HTTP/WS--> J-Prime (Mind) <--sandbox--> Reactor Core (Nerves)
     |
...
### 24 Autonomous Sensors

AuditDrift, Backlog, CageHygiene, CapabilityGap, CrossRepoDrift, CUExecution, DeepAnalysis, DocStaleness, GitHubIssue, IntentDiscovery, Liveness, MemoryHygiene, Meta, OpportunityMiner, PerformanceRegression, ProactiveExploration, RuntimeHealth, ScheduledTrigger, TestFailure, TodoScanner, Vision, VoiceCommand, WebIntelligence, WorkOrder.

> This list drifted to 16 while the tree held 24 — the eight missing were AuditDrift, CageHygiene, DeepAnalysis, Liveness, MemoryHygiene, Meta, Vision and WorkOrder. Derive it, don't retype it: `grep -hoE '^class [A-Za-z0-9_]*Sensor\b' backend/core/ouroboros/governance/intake/sensors/*.py | sort -u`.
...
- **ToolRenderRegistry — CLOSED 2026-05-04** (`battle_test/tool_render_{registry,policy,store,view}.py`): Descriptor-driven adaptive tool-result rendering replacing two hardcoded `if/elif` paths. `BoundedBodyStore` ring with monotonic `t-N` refs (env `JARVIS_TOOL_RENDER_STORE_SIZE`, default 50). Master `JARVIS_TOOL_RENDER_REGISTRY_ENABLED` default-TRUE (legacy paths byte-identical rollback). `tool_render_view.py` is the only Rich-importing layer. 275 tests. Details: `memory/project_gap_2_tool_render_registry.md`.
- **Presentation Restraint — CLOSED 2026-05-04** (`battle_test/{presentation_restraint,repl_completion,repl_input_polish}.py`): CC-style aesthetic restraint (emojis kept). Minimal boot Panel (`/preflight` + `/organism` verbs hold detail); `chrome_color()` reserves green for outcomes; always-visible idle breadcrumb. **Load-bearing TTY fix**: `should_render()` uses `real_stdout_isatty` (`sys.__stdout__`) not `sys.stdout.isatty()` which fails under `patch_stdout(raw=True)`. Auto-discovered slash palette (27 verbs via `inspect.getmembers`), FileHistory + Ctrl+R, `@filepath` mentions, Esc-to-cancel, OSC 0 title. Masters `JARVIS_PRESENTATION_RESTRAINT_ENABLED`/`JARVIS_REPL_COMPLETION_ENABLED`/`JARVIS_REPL_INPUT_POLISH_ENABLED` default-TRUE. 198 tests. Details: `memory/project_gap_7_presentation_restraint.md`.
- **NarrativeChannel — CLOSED 2026-05-04** (`battle_test/{narrative_channel,narrative_renderer}.py` + `governance/{intent_prompter,tool_preamble_synthesizer}.py`): Makes O+V's proactive reasoning legible via the model's voice surface. `NarrativeKind` (6) taxonomy; `n-N` refs. Three surfaces: bounded fire-and-forget intent LLM call (`💭`), deterministic tool-WHY preamble (`🗣`), closed-table visual hierarchy. `/narrate {off|preambles|on|verbose}`; `/expand n-N`. Masters `JARVIS_NARRATIVE_INTENT_ENABLED` + `JARVIS_TOOL_PREAMBLE_FALLBACK_ENABLED` default-TRUE. 175 tests. Details: `memory/project_gap_6_narrative_channel.md`.
- **Live Status Line + Collapsible Op Blocks — CLOSED 2026-05-04** (`battle_test/{live_status_line,op_block_buffer}.py`): Closes audit Gap #1 (persistent status line) + #3 (collapsible op blocks); Gap #5 re-diagnosed as phantom. Wraps `StatusLineBuilder.render_plain()` into `PromptSession(bottom_toolbar=...)`; `OpBlockBuffer` FIFO ring with `o-N` refs (env `JARVIS_OP_BLOCK_BUFFER_SIZE`, default 50). Unified `/expand <ref>` dispatches `t-N`/`d-N`/`o-N`. Masters `JARVIS_LIVE_STATUS_LINE_ENABLED` + `JARVIS_OP_COLLAPSE_ENABLED` default-TRUE. 108 tests. Details: `memory/project_gap_135_live_status_collapse.md`.
- **IDE-native ReviewBranch — CLOSED 2026-05-04** (`battle_test/diff_archive.py` + `governance/{review_branch_manager,review_coordinator}.py`): Yellow-tier (NOTIFY_APPLY) diff parity via non-destructive git preview branches (`hash-object`+`commit-tree`+`branch` — never touches working tree/HEAD/index) surfaced in VS Code source control. `DiffArchive` ring (`d-N` refs, env `JARVIS_DIFF_ARCHIVE_SIZE`, default 30); `ReviewCoordinator` façade + per-op `asyncio.Event`. Default 300s → auto-REJECT (`JARVIS_REVIEW_TIMEOUT_S`; `=0` = legacy auto-apply). Master `JARVIS_REVIEW_BRANCH_ENABLED` default-TRUE. Verbs `/accept` `/reject` `/review`. 167 tests. Details: `memory/project_gap_4_review_branch.md`.
- **SideChannel / `/btw`** (`governance/{side_channel,btw_repl}.py`): the operator's aside lane — a question asked WHILE O+V works, that neither becomes work nor takes the floor. Root cause it closes: `/goal` turns a question into an IntentEnvelope, and `/ask` awaits the provider on `operator_input_queue._drain`'s single consumer, so the next keystroke queues behind the last answer. Three properties: **submit is instant** (returns an `s-N` ticket, never awaits a provider); **answering is non-preemptive** (`assess_admission` defers while ops are in flight past `JARVIS_BTW_OPS_HEADROOM` or memory pressure is CRITICAL, jittered exponential backoff); **deferral is bounded** (`JARVIS_BTW_DEFER_MAX_S`, default 120s, then admitted under load and the answer says so). Answers come from `fast_path_qa.ask_question` — no parallel provider, no parallel budget — grounded via its new additive `grounding=` seam with a live-situation digest composed from a reader REGISTRY (status line / in-flight streams / narrative / recent ops / posture / queue), off-loop and time-boxed. Delivery re-enters the originating `attach_session.session_scope` (a ContextVar does not survive the task boundary) and holds while a modal overlay owns the screen. Refuses rather than drops at depth; only TERMINAL tickets are evicted; a `PRE_SHUTDOWN_EVENT_SET` termination hook reports unanswered asides. Verbs `/btw <q>` `/btw` `/btw status` `/btw cancel s-N`, plus `/expand s-N`. Reachable from BOTH surfaces because `harness._try_registry_verb` now falls through to `repl_dispatch_registry.try_dispatch` (closing the attach-vs-daemon verb split for all 89 auto-discovered verbs). Master `JARVIS_BTW_ENABLED` default-TRUE (the LANE); answering still gated by `JARVIS_FAST_PATH_QA_ENABLED`. 118 tests.
...
- **L2 Repair** (`repair_engine.py`): Iterative self-repair FSM (5 iterations, 120s timebox). **Enabled by default** (`JARVIS_L2_ENABLED=true`) — engages when VALIDATE exhausts retries, closes the Ouroboros cycle per Manifesto §6. Slice 9: sandbox baseline = working tree; real failing-test scoping; honest failure logs.
...
- **Advisor Targeted Locality Bounding + Epistemic Humility** (`advisor_locality.py` + `operation_advisor.py` `_compute_blast_radius{,_async}_ex`, 2026-07-21): Kills the cold-cache blast-radius fabrication class (soak bt-2026-07-21-205755: 39-43s global-scan burns → fabricated `blast=50` presented as measured, cache-poisoned, and satisfying hard-BLOCK via a latent Slice 21 Fix B bypass on the cooperative path). On budget exhaustion the scan pivots to a bounded O(K) localized neighborhood (target package dirs + direct importees via the canonical `reverse_dep_resolver.extract_module_imports` + test dirs) → honest MEASURED LOWER BOUND (`Advisory.blast_provenance="localized_lower_bound"`, hard-BLOCK eligible — same-or-sharper). If that too cannot resolve → provenance `unknown`: NEUTRAL blast=0, no risk factor, never cached, never BLOCK; the epistemic ledger routes a NOTIFY_APPLY floor through `_advisor_epistemic_notify_floor` on BOTH GATE paths. Cold-root memo (TTL) skips repeat global burns. Masters `JARVIS_ADVISOR_LOCALITY_BOUNDING_ENABLED` + `JARVIS_ADVISOR_EPISTEMIC_NOTIFY_ENABLED` default-TRUE (locality OFF = legacy cap, now honestly `synthetic_cap`); knobs `JARVIS_ADVISOR_LOCALITY_{TIMEOUT_S,MAX_SCANNED,MAX_ROOTS}` + `JARVIS_ADVISOR_COLD_ROOT_TTL_S`. 14-test spine `tests/governance/test_advisor_locality_epistemic.py`.
...
- **AuditRatchet sweep + AuditDriftSensor** (`audit_ratchet.sweep` + `intake/sensors/audit_drift_sensor.py`, 2026-08-15): closes the loop from *measured* to *acted on*. The ratchet's boot watchdog only ever logged one WARNING line; a regression against an accepted floor now becomes an `IntentEnvelope`. **One scan, N consumers**: `sweep()` is single-flight + TTL-cached (`JARVIS_AUDIT_SWEEP_TTL_S`, default 900s) over `run_registered_watchdogs()` — the boot seam and the sensor share the same reading (the two shipped instruments cost ~12s + ~29s of parsing), and a cached reading is re-stamped `fresh=False` so no consumer can report a scan it never ran. **Dedup is structural, not statistical**: findings cluster by their resolved DIRECTORY (`locate_finding`: `path::node` → path; dotted module → two stats, then the canonical `reverse_dep_resolver.build_module_to_path`) — no trace hashing, no correlation window. A drift large in *proportion* to `scanned` (`max(STORM_MIN=12, STORM_FRACTION=0.05 × scanned)`) collapses to ONE storm envelope. Subset buckets claim their keys first (evidence `flag_literal` ⊂ `source_only`), unlocatable findings never become ops (counted in `health()`), and every envelope is `urgency=low` on BACKGROUND. Event-primary via `fs.changed.*` with a settle timer (`JARVIS_AUDIT_DRIFT_SETTLE_S`, 300s) coalescing an editing session into one measurement; poll is the backstop. **The floor is not an editable target** — `.jarvis/` plus `audit_ratchet`/`surface_reachability`/`source_assertion_audit`/`audit_drift_sensor` are in `tool_executor._PROTECTED_PATH_SUBSTRINGS`, so the cheapest "fix" (re-accept the baseline, weaken the audit) is structurally refused. Master `JARVIS_AUDIT_DRIFT_SENSOR_ENABLED` default **false**. Token bucket + per-scan cap shared with the cage/liveness sensors via `intake/sensors/emission_control.py`. 31 tests.
...
- **TestRunner** (`test_runner.py`): Multi-strategy async test discovery + pytest execution. Env: `JARVIS_TEST_TIMEOUT_S`, `JARVIS_TEST_RETRY_ENABLED`, `JARVIS_TEST_MAX_FILES`, `JARVIS_TEST_DIR_NAMES`. Known fix: sandbox path resolution via original_paths mapping (commit 22f297d). Slice 8 closed a containment bypass in that same mapping: `_normalize` now proves containment FIRST (repo_root ∪ declared `sandbox_dir` ∪ legacy env prefixes) — `original_paths` selects mapping/shape only inside the proven-sandbox branch, never as an independent acceptance path around `BlockedPathError`.
- **Test→Source Attribution Bridge — Slice 6** (`intent/test_source_attribution.py` + `reverse_dep_resolver.extract_module_imports`/`build_module_to_path`): TestFailure signals carry BOTH loci — `target_files=(*source_loci, test_locus)` resolved by deterministic AST import tracing of the failing test module (alias/relative/mock.patch-target aware; traceback frames rank, never decide). Unresolvable → typed `AttributionUnresolved` fail-fast: scope stays test-locus, evidence `attribution.status=unresolved`, and the orchestrator gate escalates test-only mutations to APPROVAL_REQUIRED (kills the Run-16 blind class). Evidence block schema-versioned (`build_attribution_evidence`, mirrors Vision discipline). `Attribution.method` is one of `direct_import`, `patch_target`, or `direct_import+patch_target`. Gate (`_attribution_scope_risk_floor`) lives on BOTH GATE paths — inline orchestrator and the extracted `gate_runner.py` (the shipping default under `JARVIS_PHASE_RUNNER_GATE_EXTRACTED`). Masters: `JARVIS_TEST_SOURCE_ATTRIBUTION_ENABLED` + `JARVIS_ATTRIBUTION_SCOPE_GATE_ENABLED` default-TRUE; knobs `JARVIS_ATTRIBUTION_MAX_SOURCE_FILES` (8), `JARVIS_ATTRIBUTION_MODULE_MAP_TTL_S` (300). Slice 7: MultiFileCoverageGate applies SUBSET semantics to resolved-attribution scope (candidate covering ≥1 attributed target passes — the Run-17 `covers 1/2` blocker; zero-coverage still rejected, non-attributed multi-file ops keep the superset demand; evidence forwarded on BOTH GENERATE paths with an AST wiring pin). Master: `JARVIS_ATTRIBUTION_SUBSET_COVERAGE_ENABLED` default-TRUE. Slice 8: declared-roots containment contract in the VALIDATE router (`test_runner._normalize` honors the caller-declared `sandbox_dir`/`original_paths` — kills the Run-18 `BlockedPathError→security` class under the node's no-`/tmp` policy and restores directory-shape adapter routing for sandbox copies); resolved-attribution containment at the coverage gate (`JARVIS_ATTRIBUTION_CONTAINMENT_ENABLED`) + test-only NOTIFY_APPLY floor on BOTH GATE paths (`JARVIS_ATTRIBUTION_TEST_ONLY_NOTIFY_ENABLED`); failed VALIDATE results logged at the shared `_run_validation` seam (§7); flag-audit REJECTs require same-line family corroboration (`JARVIS_A1_AUDIT_CORROBORATED_REJECTS`). Slice 9: VALIDATE exercises the candidate — working-tree-faithful candidate tree (`RepairSandbox` + dirty-delta overlay, `JARVIS_SANDBOX_WORKING_TREE_MIRROR_ENABLED`) with a per-op router anchored at the tree (`JARVIS_VALIDATE_CANDIDATE_TREE_ENABLED`, fail-soft to legacy); L2 runs the REAL failing tests with a pinned pytest config (`JARVIS_L2_TEST_TARGET_THREADING_ENABLED`; the `backend/pytest.ini` rc=4 hijack is dead) and honest rc+tail failure logs; pre-GATE `..` write-escape clamp (fc='security' before any byte lands); flag-audit rejects scoped to the chaos lineage in single-session audits (`JARVIS_A1_AUDIT_LINEAGE_SCOPED_REJECTS`).

- **Execution-root seam + WorkspacePromoter — Slice 11** (`autonomous_workspace.effective_execution_root` + `workspace_promoter.py` + `worktree_manager.promote_commits`): Closes the Run-21 composition bug (Slice 56 redirected APPLY+commit into the ledger-sovereignty workspace while VERIFY judged the boot `project_root` → correct repairs structurally failed `verify_regression`; no promotion path existed by design). Role split: `project_root` = observation (sensors/TestWatcher), dynamic `config.execution_root` (lazy property on BOTH `GovernedLoopConfig` and `OrchestratorConfig`, resolved at read time through the ONE canonical seam) = mutation/judgment. VERIFY (scoped tests via per-root `LanguageRouter` — `_scoped_verify_runner`, Slice-9 discipline since PythonAdapter ignores `sandbox_dir`; PatchBenchmarker; rollback; Gate-2 blast radius; containment probe) anchors the execution root on BOTH duplicated VERIFY paths. Post-AutoCommit, `run_workspace_promotion` (8b→8b2 seam, both paths) lands the verified commit on the operator tree: LiveWork consult (reused gate) + GENERATE-hash drift re-check (reused `state_drift`) + `WorktreeManager.promote_commits` (async cherry-pick / `merge --ff-only`, typed fail-closed `PromotionError` states, conflict→abort→byte-identical target, never force). Masters: `JARVIS_WORKSPACE_PROMOTION_ENABLED` default **false** (prod = quarantine + Orange PR; A1 driver opts in), `JARVIS_PROMOTION_LIVE_WORK_CONSULT`, `JARVIS_PROMOTION_MAX_COMMITS` (8), `JARVIS_PROMOTION_REQUIRE_CLEAN_TARGETS`. Flag-audit hardening rides along: full op-ids at audit-keyed REJECT emits + auditor `resolve_op_candidates` all-outside ambiguity excusal (UUIDv7 same-ms truncation class).

- **Slice 12+13 — landing + value** (`worktree_manager` baseline exemption + `candidate_value_gate.py`): Slice 12 lets a verified repair LAND on the defect it fixes — a dirty promotion-target path is exempt from `target_dirty` IFF its live sha256 exactly equals the op's GENERATE-time baseline (`state_drift.file_sha256`, the drift check's own object — proof the dirt IS the defect state, not human work; archive→`git restore`→land; any deviation/deletion/lock fails closed; `JARVIS_PROMOTION_BASELINE_EXEMPTION_ENABLED`). Slice 13 kills the Run-22 noise class — at the post-GENERATE seam, a candidate whose every file is mathematically cosmetic (Python: AST equality after `epistemic_shedder` docstring stripping; requirements/cfg/ini: whole-line grammar normalization) completes as benign `no_op_cosmetic` before VALIDATE/APPLY/VERIFY spend; one byte of executable change, a SyntaxError, or an unknown format passes forward (`JARVIS_CANDIDATE_VALUE_GATE_ENABLED`).
...
Boots the full 6-layer stack: GovernedLoopService, IntakeLayer (24 sensors), TrinityConsciousness, StrategicDirection, CommProtocol, SerpentFlow CLI.
...
> **Watchdog Isolation Invariant (Slice 47, 2026-05-30 — load-bearing).** The wall-clock cap and its resource-zero hard-kill thread (`harness.py::_start_wall_clock_hard_deadline_thread`) MUST remain completely blind to application state-ledgers — they read only raw `time.monotonic()`/`time.time()`, never the Orchestrator op-ledger, phase, or any liveness signal from the system they guard. **Rationale:** Slice 47 considered an "adaptive budget waiver" that would extend the cap when a worker reached APPLY/VERIFY/AutoCommit. Rejected on bisection: coupling an out-of-band watchdog to the inner op-ledger means a *wedged* VERIFY (deadlock, pytest hang — the v41 67-min failure class) keeps the extend-condition true forever, so the watchdog deadlocks *with* the system instead of killing it. This is the same principle that retired the `JARVIS_WALL_CLOCK_ESCALATION_SIGTERM_S/_EXIT_S` knobs (`harness.py:5489`): a watchdog that shares a resource — signal path, logging lock, **or state-ledger** — with the system it guards is not a watchdog. The correct, decoupled answer to "the patch got severed mid-commit" is a larger *static* margin (`--max-wall-seconds 2400`), never a dynamic activity-gated waiver.
...
backend/core/ouroboros/
  governance/
    ...
    intake/
      sensors/                  # 24 sensors (count derived from the *Sensor classes here)
...
Full postmortems live in `docs/architecture/OUROBOROS.md#battle-test-breakthrough-log`. The canonical source of truth for any "did the loop work" question is the session `debug.log` under `.ouroboros/sessions/<session-id>/`, not `summary.json` (which has a known `attempted` counter bug).

Per-arc graduation details and session-level findings are tracked in `memory/project_*.md` (one file per arc). MEMORY.md indexes them.
