...
- **2026-04-20 (Gap #6 — CLOSED, 4-slice arc)** — IDE integration surface. Slice 1: read-only GET endpoints `/observability/{health,tasks,tasks/{op_id}}` on `EventChannelServer` (loopback-only + rate-limit + CORS allowlist + authority-import-ban). Slice 2: SSE `/observability/stream` + `StreamEventBroker` (10-event vocabulary + bounded caps + drop-oldest + `Last-Event-ID` replay + heartbeat + task_tool publish hooks). Slice 3: VS Code / Cursor TypeScript extension (`extensions/vscode-jarvis/`) — Tree View + CSP-hardened webview + status bar + command palette; native `fetch`+`ReadableStream` SSE parser; exp-backoff+jitter reconnect; bounded LRU. Slice 4 **graduation**: `JARVIS_IDE_OBSERVABILITY_ENABLED` + `JARVIS_IDE_STREAM_ENABLED` both flipped `false`→`true`; 18 graduation pins (10 obs + 8 stream) covering full-revert matrix + authority invariant + loopback/rate/CORS persistence + docstring bit-rot + unidirectional-transport (GET-only registration). Cursor is zero-change compatible (same `.vsix`, existing `vscode-webview://` CORS pattern already matches). **125 Python tests green** (41 obs + 49 stream + 35 VS Code TS). Sublime Text plugin deferred to a future slice. Details: `memory/project_gap_6_slice{1,2,3}_*.md` + `project_gap_6_slice4_graduation.md`.
