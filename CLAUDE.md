...
- **2026-04-20 (Gap #5 Slices 1+2+3 — TaskBoard: primitive, Venom tools, advisory prompt)** — "Structured to-do lists / what am I working on right now" view. Slice 1: `task_board.py` primitive — per-op ephemeral `TaskBoard` + frozen `Task`; state machine pending→{in_progress,completed,cancelled} with terminal-sticky + single-focus invariant (at most one in_progress); Option A lifecycle (lazy ctx-attach, explicit `close()`, NO `__del__` — grep-enforced); bounded capacity (env-tunable, captured at board birth); stable IDs `task-{op_id}-{seq:04d}`; §8 audit via synchronous per-transition INFO logs `[TaskBoard] task_{created,started,completed,cancelled,updated} op=X task_id=Y seq=N`; post-close mutations raise `TaskBoardClosedError`. Slice 2: `task_tool.py` + Rule 17 in `GoverningToolPolicy.evaluate` + 3 manifests (`task_create`/`task_update`/`task_complete`), all with `capabilities=frozenset()` (scratchpad, not mutation, not in `_MUTATION_TOOLS`, allowed under is_read_only); deny-by-default via `JARVIS_TOOL_TASK_BOARD_ENABLED` (mirrors Ticket #4); shared `classify_task_args` validator + per-op `_BOARDS` registry + single canonical `close_task_board(op_id, reason)` API. Slice 3: `TaskBoard.render_prompt_section()` → `## Current tasks (advisory)` subsection with "Not authoritative" preamble, Tier -1 sanitization via `sanitize_for_log`, `<redacted>` fallback on sanitizer-strip (don't fight the sanitizer blindly); env default ON (`JARVIS_TASK_BOARD_PROMPT_INJECTION_ENABLED=true`); orchestrator `finally:` hook invokes `close_task_board(ctx.op_id, reason=<phase>)` alongside CostGovernor/ForwardProgress cleanup; CONTEXT_EXPANSION injection mirrors SemanticIndex pattern (lazy read from registry, no lazy-create — no empty subsection on ops that didn't touch task tools). Authority invariants grep-enforced at every slice: module doesn't import iron_gate / risk_tier_floor / semantic_guardian / policy_engine. Manifesto §1 + §6 + §8 preserved throughout. **90/90 tests green** (33 Slice 1 + 35 Slice 2 + 22 Slice 3). Slice 4 owns default flip + live-fire. Details: `memory/project_gap_5_{taskboard_slice1,task_tool_slice2,slice3_prompt_injection}.md`.
