...
- **2026-04-20 (Phase C Epoch 3 Slices 3a+3c — Semantic Index v1.0)** — First Synthetic Soul (§4) expansion after the cage sealed. Slice 3a: hand-rolled NumPy k-means (no sklearn dep) + auto-K silhouette discovery + cluster-kind classifier (`goal`/`conversation`/`postmortem`/`mixed`) + shadow-mode alignment observation + failure-gravity tripwire (rolling 50-signal window, 0.3 threshold, WARN + counter). `score()` return unchanged under `cluster_mode=kmeans` — telemetry only. Slice 3c: `format_prompt_sections()` renders per-cluster `### Theme: <label> (N items, <kind>)` blocks (operators SEE themes before policy changes). Deterministic tokenizer for labels (no LLM). K=1 degrades to v0.1 Focus items. Postmortem-kind clusters rendered as themes w/ kind tag. Authority invariant preserved (import-surface test). `JARVIS_SEMANTIC_INDEX_CLUSTER_MODE` default `centroid` (shadow); flips to `kmeans` after 3 clean sessions + coherent themes. **91/91 semantic-index tests green**. Details: `memory/project_phase_c_semantic_index_v1.md`.
