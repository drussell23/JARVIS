...
- **ToolRenderRegistry — CLOSED 2026-05-04** (`battle_test/tool_render_{registry,policy,store,view}.py`): Descriptor-driven adaptive tool-result rendering replacing two hardcoded `if/elif` paths in `serpent_flow.op_tool_call` + `ouroboros_tui.show_tool_call`. Closed `BodyShape` (6) + `ToolStatus` (4) + `DensityLevel` (3) + `LayoutKind` (3) taxonomies; declarative `(Posture × LayoutKind) → DensityLevel` table covering all 12 cells; `BoundedBodyStore` ring with monotonic `t-N` refs (no reuse) + drop-oldest eviction (env: `JARVIS_TOOL_RENDER_STORE_SIZE`, default 50); `Default*Provider` lazy-import DI cage. Master switch `JARVIS_TOOL_RENDER_REGISTRY_ENABLED` graduated default-TRUE; legacy paths preserved verbatim below master-flag guards for byte-identical rollback. 3 AST pins (view public surface / descriptor completeness / policy DI cage). 275 regression tests. Details: `memory/project_gap_2_tool_render_registry.md`. Slice 4's `tool_render_view.py` is the **only** Rich-importing layer below the call sites — substrate stays renderer-agnostic.
- **Presentation Restraint — CLOSED 2026-05-04** (`battle_test/{presentation_restraint,repl_completion,repl_input_polish}.py`): CC-style aesthetic restraint applied to O+V's CLI while preserving identity (emojis kept). **Boot redesign** (Slice 1): minimal `Rich.Panel` (~10 lines vs legacy ~30) replaces multi-section dashboard; preflight + 6-layer organism block move behind `/preflight` + `/organism` REPL verbs; `jarvis.shutdown.diagnostics` INFO root-propagation suppressed. **Color discipline + idle status** (Slice 2): `chrome_color()` returns `dim` under restraint so green stays reserved for outcomes (✨ evolved, ✓ test passed); `format_idle_breadcrumb()` always-visible breadcrumbs (`IDLE · main · $0.04/$0.50 · EXPLORE`); **load-bearing TTY gate fix** — `should_render()` now uses `real_stdout_isatty` (checks `sys.__stdout__`, the unpatched original) instead of `sys.stdout.isatty()` which fails under `patch_stdout(raw=True)` (this is why the Gap #1+5 status line silently never surfaced before). **Auto-discovered slash palette** (Slice 3): `inspect.getmembers` walks `SerpentREPL._handle_*` methods → 27 verbs registered structurally (no hardcoded list). `prompt_toolkit.completion.Completer` fires only on `/` prefix; `prompt_toolkit.history.FileHistory` at `.jarvis/repl_history` + Ctrl+R reverse-search. **Input polish** (Slice 4): `@filepath` mention extraction (whitespace-boundary regex + slash-or-extension heuristic filters out emails + Python decorators), Esc-to-cancel (buffer-empty filter — Esc cancels active op when operator is waiting, preserves typing otherwise), OSC 0 terminal title (TERM-aware, escape-sanitized, length-bounded with ellipsis). Three master switches `JARVIS_PRESENTATION_RESTRAINT_ENABLED` + `JARVIS_REPL_COMPLETION_ENABLED` + `JARVIS_REPL_INPUT_POLISH_ENABLED` graduated default-TRUE; legacy paths preserved verbatim under `=false` rollback. 6 FlagRegistry seeds + 5 AST pins (default-true persistence regression / boot_banner short-circuit / `_loop` wiring of completion + polish / op-lifecycle title hooks / `status_line.should_render` uses `real_stdout_isatty`). 198 regression tests. Details: `memory/project_gap_7_presentation_restraint.md`.
- **NarrativeChannel — CLOSED 2026-05-04** (`battle_test/{narrative_channel,narrative_renderer}.py` + `governance/{intent_prompter,tool_preamble_synthesizer}.py`): Makes O+V's proactive intelligence legible via the model's voice surface. Closes the operator-flagged "silent black box" gap — for a proactive system (sensors fire ops without prior user prompt) the model's narrative reasoning must be present to anchor operator context. Closed `NarrativeKind` (6) taxonomy: `INTENT`/`PLAN_PROSE`/`TOOL_PREAMBLE`/`THINKING`/`L2_REPAIR_PROSE`/`POSTMORTEM_PROSE`. `NarrativeChannel` substrate with monotonic `n-N` refs and composite-key active-frame index supporting parallel kinds per op (THINKING + PLAN_PROSE interleaved). **Three operator-facing surfaces:** (1) `op_started` fires a fire-and-forget bounded LLM call (Tier 0 DW, 50-token cap, 5s timeout, hard-fail-silent) for "I'm going to do X" intent — surfaced as `💭` bright_blue italic above the op block; (2) `op_tool_start` synthesizes a deterministic 1-sentence WHY (per-tool descriptor table, no LLM cost) when the model omits a preamble — Tool Transparency: every tool call gets `🗣` bright_black italic line; (3) closed-table visual hierarchy distinguishes model voice from system actions (bright_blue/bright_black for prose vs cyan for `⏺` actions). REPL `/narrate {off|preambles|on|verbose}` controls density via env-flag composition. Unified `/expand <ref>` dispatcher extended with `n-N` prefix. Master flags `JARVIS_NARRATIVE_INTENT_ENABLED` + `JARVIS_TOOL_PREAMBLE_FALLBACK_ENABLED` graduated default-TRUE. 5 FlagRegistry seeds + 4 AST pins (NarrativeKind taxonomy frozen / renderer covers all kinds / op_tool_start synthesizer wired regression pin / op_started intent prompt wired regression pin). 175 regression tests. Details: `memory/project_gap_6_narrative_channel.md`.
- **Live Status Line + Collapsible Op Blocks — CLOSED 2026-05-04** (`battle_test/{live_status_line,op_block_buffer}.py`): Closes the original UX audit's Gap #1 (no persistent status line during generation) + Gap #3 (no collapsible op blocks). **Gap #5 was re-diagnosed as a phantom** — `patch_stdout(raw=True)` already handles async output interleaving during typing; what was missing was the status line itself (Gap #1). Slice 1 wraps the existing `StatusLineBuilder.render_plain()` (already registered by `harness.py`) via `make_bottom_toolbar_callable` and plumbs it into `PromptSession(bottom_toolbar=...)` — zero new rendering surface. Slice 2 ships `OpBlockBuffer`: closed `OpBlockState` (3) taxonomy, per-op FIFO ring with monotonic `o-N` refs (env: `JARVIS_OP_BLOCK_BUFFER_SIZE`, default 50), active-index pruning on eviction so concurrent appends after eviction safely fail rather than misroute. Slice 3 wires `op_started`/`_op_line`/`op_completed`/`op_failed` lifecycle hooks + unified `/expand <ref>` REPL verb dispatching across all THREE artifact substrates by ref prefix (`t-N` → tool bodies from Gap #2, `d-N` → diffs from Gap #4, `o-N` → op blocks from this arc, bare op-id → most recent `o-N`). Both master switches `JARVIS_LIVE_STATUS_LINE_ENABLED` + `JARVIS_OP_COLLAPSE_ENABLED` graduated default-TRUE. 4 AST pins (status-line wiring regression / `OpBlockState` frozen / lifecycle hooks present on all 4 methods / 3-prefix `/expand` dispatch). 108 regression tests. Deliberately rejected: full prompt_toolkit Application+Layout migration (unnecessary once status line is wired). Details: `memory/project_gap_135_live_status_collapse.md`.
- **IDE-native ReviewBranch — CLOSED 2026-05-04** (`battle_test/diff_archive.py` + `governance/{review_branch_manager,review_coordinator}.py`): IDE-native diff parity for Yellow-tier (NOTIFY_APPLY) ops. Replaces the 5s Rich overlay → auto-apply path with non-destructive git preview branches (`hash-object` + `commit-tree` + `branch` plumbing — NEVER touches working tree, HEAD, or operator's index) that VS Code's source control panel surfaces natively. Closed `DiffOutcome` (5) + `VerifyOutcome` (4) + `ReviewState` (5) + `ReviewDecision` (5) + `CreateOutcome`/`AcceptOutcome`/`RejectOutcome` taxonomies. `DiffArchive` ring (monotonic `d-N` refs, env: `JARVIS_DIFF_ARCHIVE_SIZE`, default 30) carries full lifecycle audit (apply/verify outcomes are terminal-frozen). `ReviewCoordinator` façade joins archive + branch manager + per-op `asyncio.Event` rendezvous + cancel-check polling. Default 300s timeout → auto-REJECT (env: `JARVIS_REVIEW_TIMEOUT_S`; `=0` opts back to legacy auto-apply). Master switch `JARVIS_REVIEW_BRANCH_ENABLED` graduated default-TRUE; legacy 5s overlay path preserved verbatim below master-flag guard for byte-identical rollback. 4 SSE event types (`review_branch_created`/`accepted`/`rejected`/`expired`); REPL verbs `/accept` `/reject` `/review`. 4 AST pins (ReviewState frozen / DiffOutcome+VerifyOutcome frozen / orchestrator hook regression / SerpentREPL handlers complete). 167 regression tests. Slice 5 (VS Code custom command + webview) deferred — branches surface natively in source control without it. Details: `memory/project_gap_4_review_branch.md`.
