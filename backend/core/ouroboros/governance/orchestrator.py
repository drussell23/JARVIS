...
class GovernedOrchestrator:
    ...
    async def _run_pipeline(self, ctx: OperationContext) -> OperationContext:
        ...
        # VALIDATE has ONE implementation: VALIDATERunner (also what
        # dispatch_pipeline registers). The inline copy that sat behind
        # JARVIS_PHASE_RUNNER_VALIDATE_EXTRACTED=false was deleted after it
        # drifted from the runner twice in one soak day (sibling verdicts;
        # a micro-fix success that advanced GATE twice and never
        # re-validated) -- a rollback path to a broken copy is not a
        # safety net. The best_candidate local leaks downstream to GATE;
        # threaded via PhaseResult.artifacts.
    ...
    async def _publish_candidate_verdict(
        ...
        *,
        ...
        The inline copy is gone now (VALIDATE has one implementation,
        VALIDATERunner); this stays the ONE publisher both sinks share.
