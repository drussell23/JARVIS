...
async def check_push_fan_out() -> Tuple[bool, str]:
    """Slice 5 proof: a single HTTP push delivery fans out to BOTH
    DocStalenessSensor and CrossRepoDriftSensor concurrently. The
    dispatcher replaced the earlier if/elif short-circuit so no sensor
    is starved.
    """
    import json
    try:
    ...
    prev_doc = os.environ.get("JARVIS_DOC_STALENESS_WEBHOOK_ENABLED")
    prev_drift = os.environ.get("JARVIS_CROSS_REPO_DRIFT_WEBHOOK_ENABLED")
    prev_ch = os.environ.get("JARVIS_EVENT_CHANNELS_ENABLED")
    os.environ["JARVIS_DOC_STALENESS_WEBHOOK_ENABLED"] = "true"
    os.environ["JARVIS_CROSS_REPO_DRIFT_WEBHOOK_ENABLED"] = "true"
    os.environ["JARVIS_EVENT_CHANNELS_ENABLED"] = "true"
    ...
    try:
        ...
        doc = DocStalenessSensor(
            ...
            project_root=REPO_ROOT, scan_paths=("backend/core/",),
        )
        ...
        try:
            ...
            # Payload touches a file watched by BOTH sensors:
            # - DocStaleness: .py under backend/core/ scan path
            # - CrossRepoDrift: op_context.py is in _WATCHED_PATHS
            payload = {
                "ref": "refs/heads/main",
                "commits": [{
                    "id": "fan-out-ov-smoke",
            ...
            if len(doc_calls) != 1:
                return False, (
                    f"DocStaleness must run exactly once, got {len(doc_calls)}"
                )
            if len(drift_calls) != 1:
                return False, (
                    f"CrossRepoDrift must run exactly once, got {len(drift_calls)}"
                )

            return True, (
                f"PASS — fan-out to both sensors (doc={len(doc_calls)} "
                f"drift={len(drift_calls)}) via asyncio.gather on port {port}"
            )
        finally:
            await server.stop()
    finally:
        for key, prev in (
            ("JARVIS_DOC_STALENESS_WEBHOOK_ENABLED", prev_doc),
            ("JARVIS_CROSS_REPO_DRIFT_WEBHOOK_ENABLED", prev_drift),
            ("JARVIS_EVENT_CHANNELS_ENABLED", prev_ch),
        ):
            if prev is None:
                os.environ.pop(key, None)
