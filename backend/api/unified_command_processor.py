...
class UnifiedCommandProcessor:
    ...
    async def _compose_workspace_response(
        ...
        _das_canary_raw = f"{_cmd_id}:{_normalized_cmd}"
        das_canary_key = hashlib.sha256(_das_canary_raw.encode()).hexdigest()
        logger.debug("[DAS] canary_key=%s cmd_id=%s", das_canary_key[:12], _cmd_id[:8])
    ...
    async def _try_workspace_fast_path(
        ...
        _normalized_cmd = re.sub(r"\s+", " ", command_text.lower().strip())
        _das_canary_raw = f"{command_id}:{_normalized_cmd}"
        das_canary_key = hashlib.sha256(_das_canary_raw.encode()).hexdigest()
        logger.debug("[DAS] canary_key=%s cmd_id=%s", das_canary_key[:12], command_id[:8])
        response_stub = SimpleNamespace(
            intent="action",
            domain="workspace",
