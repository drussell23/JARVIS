...
class JarvisCU:
    ...
    def __init__(self) -> None:
        ...
        # Try to open SHM for live 60fps frames; graceful fallback if absent
        self._shm: Optional[ShmFrameReader] = None
        try:
            reader = ShmFrameReader()
            if reader.open():
                self._shm = reader
                logger.info("[JarvisCU] SHM frame reader attached")
            else:
    ...
    def _get_frame(self) -> np.ndarray:
        """Get the latest screen frame from SHM, or a black fallback."""
